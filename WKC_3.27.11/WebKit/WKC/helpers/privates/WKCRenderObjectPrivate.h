...
public:
    WKCRingRectsPrivate(WTF::Vector<WKCFloatRect>&);
    ~WKCRingRectsPrivate();
    int length() const;
    WKCFloatRect getAt(int) const;
private:
    WKCRingRectsPrivate(const WKCRingRectsPrivate&);
    WKCRingRectsPrivate& operator=(const WKCRingRectsPrivate&);
private:
    WTF::Vector<WKCFloatRect>* m_rects;
};
...
public:
    ...
    void focusRingRects(WTF::Vector<WKCFloatRect>&);
    WKCFloatRect absoluteClippedOverflowRect();
