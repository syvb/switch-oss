...
Element*
FocusControllerPrivate::findNextFocusableElement(const FocusDirection& direction, const WKCRect* scope, Element* base, Node* container)
{
    ...
    WebCore::Node* containerWebCore = container ? container->priv().webcore() : nullptr;
    if (scope) {
        WebCore::IntRect r(scope->fX, scope->fY, scope->fWidth, scope->fHeight);
        element = m_webcore->findNextFocusableElement(webcore_dir, &r, baseWebCore, containerWebCore);
    } else {
        element = m_webcore->findNextFocusableElement(webcore_dir, 0, baseWebCore, containerWebCore);
    }
    if (!element)
        return 0;
...
Element* FocusController::findNextFocusableElement(const FocusDirection& direction, const WKCRect* scope, Element* base, Node* container)
{
    return m_private.findNextFocusableElement(direction, scope, base, container);
}
