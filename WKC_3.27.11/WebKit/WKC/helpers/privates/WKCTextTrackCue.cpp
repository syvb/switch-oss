...
int
TextTrackCuePrivate::calculateComputedLinePosition() const
{
    return (static_cast<WebCore::VTTCue*>(m_webcore))->calculateComputedLinePosition();
}
...
{
    struct Visitor {
        double operator()(double l) { return l; }
        double operator()(WebCore::VTTCue::AutoKeyword) { return std::numeric_limits<double>::quiet_NaN(); }
    };
    const auto& line = (static_cast<WebCore::VTTCue*>(m_webcore))->line();
...
int
TextTrackCue::calculateComputedLinePosition() const
{
    return m_private.calculateComputedLinePosition();
}
int
