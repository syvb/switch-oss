...
NodePrivate::hasTagName(int id) const
{
    ...
    case HTMLNames_objectTag:
        return webcore()->hasTagName(WebCore::HTMLNames::objectTag);
    ...
    case HTMLNames_labelTag:
        return webcore()->hasTagName(WebCore::HTMLNames::labelTag);
    default:
        return false;
    }
}
...
bool
NodePrivate::isDocumentNode() const
{
    return webcore()->isDocumentNode();
}
...
bool
NodePrivate::isConnected() const
{
    return webcore()->isConnected();
}
...
Node*
NodePrivate::shadowRoot()
{
    WebCore::Node* n = webcore()->shadowRoot();
    if (!n)
        return 0;
    if (!m_shadowRoot || m_shadowRoot->webcore() != n) {
        delete m_shadowRoot;
        m_shadowRoot = NodePrivate::create(n);
    }
    return &m_shadowRoot->wkc();
}
...
Node*
NodePrivate::nextSibling()
{
    if (!webcore())
        return 0;

    WebCore::Node* n = webcore()->nextSibling();
    if (!n)
        return 0;
    if (!m_nextSibling || m_nextSibling->webcore()!=n) {
        delete m_nextSibling;
        m_nextSibling = create(n);
    }
    return &m_nextSibling->wkc();
}
...
NodePrivate::getElementsByTagName(const String& localName)
{
    ...
    RefPtr<WebCore::NodeList> list = ((WebCore::ContainerNode *)m_webcore)->getElementsByTagName(WTF::AtomString(localName));
    if (!list)
        return 0;
...
NodePrivate::querySelector(const String& selectors)
{
    ...
    auto queryResult = ((WebCore::ContainerNode *)m_webcore)->querySelector(selectors);
    if (queryResult.hasException())
        return 0;
    WebCore::Element* elem = queryResult.returnValue();
    if (!elem)
        return 0;
...
NodePrivate::querySelectorAll(const String& selectors)
{
    ...
    auto list = ((WebCore::ContainerNode *)m_webcore)->querySelectorAll(selectors);
    if (list.hasException())
        return 0;
    NodeListPrivate* nodeList = new NodeListPrivate(list.releaseReturnValue());
    return &nodeList->wkc();
}
...
bool
NodePrivate::hasEditableStyle() const
{
    return webcore()->hasEditableStyle();
}
...
{
    auto result = webcore()->setTextContent(text);
...
WKCFloatRect
NodePrivate::absoluteCompositeTransformedRect(bool ignoreBorder, bool clip, bool fastPath) const
{
    WebCore::LayoutRect r = m_webcore->absoluteCompositeTransformedRect(ignoreBorder, clip, fastPath);
    return WebCore::FloatRect(r);
}
...
bool
Node::isDocumentNode() const
{
    return priv().isDocumentNode();
}
...
Node*
Node::shadowRoot() const
{
    return priv().shadowRoot();
}
...
bool
Node::hasEditableStyle() const
{
    return priv().hasEditableStyle();
}
String
...
WKCFloatRect
Node::absoluteCompositeTransformedRect(bool ignoreBorder, bool clip, bool fastPath) const
{
    return m_private.absoluteCompositeTransformedRect(ignoreBorder, clip, fastPath);
}
