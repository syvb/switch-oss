...
GraphicsLayerPrivate::clear(const WKCRect& rect, void* alternative_drawcontext)
{
    ...
    if (!dc)
        return;
    PlatformGraphicsContext *gc = static_cast<PlatformGraphicsContext *>(dc);
    gc->save();
#if USE(WKC_CAIRO)
    ...
    gc->setCompositeOperation(WebCore::CompositeOperator::SourceOver);
...
{
    Ref<WebCore::GraphicsLayer>& layer = m_webcore->children().at(index);
    if (!layer.ptr())
        return 0;
    return &(GraphicsLayerWKC_wkc(layer.ptr())->wkc());
}
...
GraphicsLayerPrivate::blendMode() const
{
    ...
    WebCore::RenderLayer* renderLayer = client.owningLayerPtr();
    if (!renderLayer)
        return toWKCBlendMode(WebCore::BlendMode::Normal);
    return toWKCBlendMode(renderLayer->blendMode());
}

bool
GraphicsLayerPrivate::requiresCompositingForWillChange() const
{
    WebCore::RenderLayer* renderLayer = m_webcore->client().owningLayerPtr();
    if (!renderLayer)
        return false;
    WebCore::RenderView* view = renderLayer->renderer().document().renderView();
    if (!view)
        return false;
    return view->compositor().requiresCompositingForWillChange(renderLayer->renderer());
}
...
GraphicsLayerPrivate::flushCompositingState(const WebCore::FloatRect& clipRect)
{
    ...
    for (size_t i = 0; i < children_size; i++) {
        Ref<WebCore::GraphicsLayer>& layer = m_webcore->children().at(i);
        if (!layer.ptr()) {
            ASSERT(false);
            continue;
        }
        GraphicsLayerPrivate* child = GraphicsLayerWKC_wkc(layer.ptr());
        child->flushCompositingState(clipRect);
    }

    updatePosition();

    m_webcore->client().didChangePlatformLayerForLayer(m_webcore);
}
...
{
    return m_webcore->contentsLayerPurpose() == WebCore::GraphicsLayer::ContentsLayerPurpose::Media;
}
...
GraphicsLayerPrivate::logReasonsForCompositing()
{
    ...
    if (!view)
        return "";
    return view->compositor().logOneReasonForCompositing(*renderLayer);
#else
    return "";
#endif
}
...
bool
GraphicsLayer::requiresCompositingForWillChange() const
{
    return m_private.requiresCompositingForWillChange();
}
