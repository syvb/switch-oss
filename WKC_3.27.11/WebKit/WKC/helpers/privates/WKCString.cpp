...
public:
    ...
    StringPrivate(const StringPrivate&);

    inline void setImpl(const WTF::RefPtr<WTF::StringImpl>& i) { if (i) m_impl = i; }
    ...
    StringPrivate& operator=(const StringPrivate&);
...
StringPrivate::StringPrivate(const StringPrivate& string)
    : m_impl(string.m_impl)
    , m_latin1(0)
    , m_utf8(0)
    , m_characters16(0)
    , m_direction(string.m_direction)
{
}
...
StringPrivate::characters16()
{
    if (!m_impl)
        return 0;
    if (!m_impl->is8Bit())
        return reinterpret_cast<const unsigned short*>(m_impl->characters16());
...
StringPrivate&
StringPrivate::operator=(const StringPrivate& orig)
{
    m_impl = orig.m_impl;
...
String::String(const unsigned short* in_str)
{
    const UChar* str = reinterpret_cast<const UChar*>(in_str);
    m_private = new StringPrivate();
    size_t len=0;
    const UChar* p = str;
    while (p && *p) {
...
String::String(const unsigned short* in_str, unsigned int len)
{
    const UChar* str = reinterpret_cast<const UChar*>(in_str);
    m_private = new StringPrivate();
    WTF::RefPtr<WTF::StringImpl> s = WTF::StringImpl::create(str, len);
...
String::String(const char16_t* str, unsigned int len)
{
    m_private = new StringPrivate();
    WTF::RefPtr<WTF::StringImpl> s = WTF::StringImpl::create(str, len);
    m_private->setImpl(s);
...
String::String(const String& str)
{
    m_private = new StringPrivate();
    if (str.m_private)
        *m_private = *str.m_private;
}

String::String(String&& str)
{
    std::swap(m_private, str.m_private);
}
...
{
    std::swap(m_private, orig.m_private);
    return *this;
}
...
void
String::append(const unsigned short* in_str)
{
    const UChar* str = reinterpret_cast<const UChar*>(in_str);
    WTF::String a(IMPL());
    a.append(str);
...
void
String::append(const unsigned short* in_str, unsigned int len)
{
    const UChar* str = reinterpret_cast<const UChar*>(in_str);
    WTF::String a(IMPL());
    a.append(str, len);
...
String&
String::replace(const unsigned short* in_a, const unsigned short* in_b)
{
    const UChar* a = reinterpret_cast<const UChar*>(in_a);
    const UChar* b = reinterpret_cast<const UChar*>(in_b);
    WTF::String s(IMPL());
    s = s.replace(a, b);
...
String&
String::replace(const unsigned short* in_a, const String& b)
{
    const UChar* a = reinterpret_cast<const UChar*>(in_a);
    WTF::String s(IMPL());
...
void
String::insert(const unsigned short* in_str, unsigned int pos, unsigned int len)
{
    const UChar* str = reinterpret_cast<const UChar*>(in_str);
    WTF::String a(IMPL());
...
String::characters() const
{
    ...
    return reinterpret_cast<const unsigned short*>(IMPL()->characters16());
}
