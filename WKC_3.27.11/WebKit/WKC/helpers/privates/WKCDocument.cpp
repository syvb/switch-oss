...
DocumentPrivate::DocumentPrivate(WebCore::Document* parent)
    ...
    , m_frame(0)
    , m_firstChild(0)
    , m_eventTrackingRegions(0)
{
}

DocumentPrivate::~DocumentPrivate()
{
    delete m_eventTrackingRegions;
    delete m_firstChild;
    delete m_frame;
    delete m_renderView;
...
bool
DocumentPrivate::isMediaDocument() const
{
    return m_webcore->isMediaDocument();
}
...
DocumentPrivate::eventTrackingRegions()
{
    auto region = m_webcore->eventTrackingRegions();
    if (!m_eventTrackingRegions || m_eventTrackingRegions->webcore()!=region) {
        delete m_eventTrackingRegions;
        m_eventTrackingRegions = EventTrackingRegionsPrivate::create(region);
    }
    return &m_eventTrackingRegions->wkc();
}
...
bool
Document::isMediaDocument() const
{
    return m_private.isMediaDocument();
}
...
EventTrackingRegions*
Document::eventTrackingRegions() const
{
    return m_private.eventTrackingRegions();
}
