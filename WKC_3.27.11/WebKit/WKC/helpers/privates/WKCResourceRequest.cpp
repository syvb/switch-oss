...
void
ResourceRequestPrivate::setFilteredURL(const KURL& url)
{
    const_cast<WebCore::ResourceRequest&>(m_webcore).setFilteredURL(url);
}
...
void
ResourceRequest::setFilteredURL(const KURL& url)
{
    m_private.setFilteredURL(url);
}
