...
found:
    ...
    for (int i=0; i<spines.size(); i++) {
        ...
        if (href.length()) {
            WTF::String s = WTF::makeString("file://", rootpath, "/", href.utf8().data());
    ...
    if (navuri && parseTOC) {
        WTF::String path = WTF::makeString(basepath, "/", navuri);
        ...
        WTF::String s = WTF::makeString("file://", rootpath, "/", navuri);
...
createNavTree(const EPUBDataNav* nav, const char* rootpath)
{
    ...
    if (count) {
        ...
        for (size_t i=0; i<count; i++) {
            ...
            WTF::String href = WTF::makeString("file://", rootpath ,"/", nav->items[i].href.utf8().data());
