...
namespace WKC {
    ...
    typedef void (*MainThreadLongjmpProc)();
    ...
    @param in_no_cache whether "Cache-Control: no-cache" exists in the HTTP header.
    @retval "!= false" Can be cached to the disk
    @retval "== false" Cannot be cached to the disk
    */
    typedef bool (*CanCacheToDiskProc)(const char* in_url, double in_current_age, double in_freshness_lifetime, long long in_original_content_size, long long in_aligned_content_size, const char* in_mime_type, bool in_no_cache);
    /**
...
*/
WKC_API bool WKCWebKitInitialize(void* memory, size_t physical_memory_size, size_t virtual_memory_size, void* font_memory, size_t font_memory_size, WKCMemoryEventHandler& memory_event_handler);
/**
...
/**
@brief Triggers a forced termination inside the browser engine
@return None
@details
This function triggers a forced termination inside the browser engine.@n
The application can call this function from any threads at any time except for executing WKCWebKitInitialize.@n
After calling this function, the application must do longjmp in the MainThreadLongjmpProc called by the browser engine.
*/
WKC_API void WKCWebKitTriggerForceTerminate();
/**
@brief Sets a callback for longjmp on the browser engine main thread
@param proc callback
@details
Sets a callback for longjmp on the browser engine main thread.@n
The callback is called by the browser engine as soon as possible after calling WKCWebKitTriggerForceTerminate.@n
The application must do longjmp in the callback.
*/
WKC_API void WKCWebKitSetMainThreadLongjmpProc(MainThreadLongjmpProc in_proc);
/**
...
*/
WKC_API void WKCWebKitRequestGarbageCollect(bool is_now = false, bool is_full = true);
/**
...
/**
@brief Gets font family name for specified font
@return Success or not
@details
Gets font family name for specified font.@n
*/
WKC_API bool WKCWebKitGetFontFamilyName(int id, char* name, int len);
/**
...
/**
@brief Run the single iteration of the run loop.
@details
Run the single iteration of the run loop on the browser engine main thread.@n
It consumes the pending tasks and expired timers, but it won't be blocked.@n
*/
WKC_API void WKCWebKitIterateRunLoop();
...
*/
WKC_API wkc_uint64 WKCWebKitGetTickCount();
...
/**
@brief Specifies ICU data.
@param data Pointer to the ICU data
@retval "!= false" Succeeded
@retval "== false" Failed
*/
WKC_API bool WKCWebKitSetICUData(const void* data);
...
public:
    ...
    */
    void setSizeForCSSDefaultViewportUnits(const WKCFloatSize& size);
    /**
       @brief small viewport
    */
    void setSizeForCSSSmallViewportUnits(const WKCFloatSize& size);
    /**
       @brief large viewport
    */
    void setSizeForCSSLargeViewportUnits(const WKCFloatSize& size);
    /**
       @brief dynamic viewport
    */
    void setLayoutViewportOverrideRect(const WKCFloatRect& rect);
    ...
    */
    ...
    void notifyPaintOffscreenFrom(const WKCRect& rect, const WKCFloatPoint& p);
    /**
    ...
    */
    bool notifyKeyEvent(WKC::KeyEventType in_type, WKC::Key in_key, unsigned int in_char, WKC::Modifier in_modifiers, bool in_autorepeat);
    ...
    */
    ...
    /**
       ...
       @retval "== false" The event is not consumed
    */
    bool notifyMouseWheel(const WKCPoint& pos, const WKCFloatSize& diff, WKC::Modifier modifiers);
    /**
    ...
    */
    ...
    void notifyPointerEvent(int type, const TouchPoint* points, int npoints, WKC::Modifier modifiers);
    /**
    ...
    */
    ...
    /**
       ...
       @param forceUserGesture forceUserGesture or not
       @param thisObject Pointer to WKC::Node instance
       @return None
    */
    void executeScriptIgnoringException(const char* script, bool forceUserGesture = false, Node* thisObject = nullptr);

    /**
       @brief Executes user JavaScript
       @param world reference to WKC::DOMWrapperWorld instance
       @param script Pointer to JavaScript string (UTF-8)
       @param forceUserGesture forceUserGesture or not
       @param thisObject Pointer to WKC::Node instance
       @return None
    */
    void executeUserAgentScriptInWorldIgnoringException(DOMWrapperWorld& world, const char* script, bool forceUserGesture, Node* thisObject = nullptr);
    ...
    */
    ...
    /**
       @brief Gets whether the caret is at the ending of the text field
       @retval "!= false" Caret is at the ending of the text field
       @retval "== false" Caret is not at the ending of the text field
       @details
       Gets whether the caret is at the ending of the text field.
       The text field may be input, textarea, or contenteditable.
    */
    bool isCaretAtEndingOfTextField() const;
    ...
    */
    ...
    static WKCFloatRect getScrollableContentRect(const WKC::Node* scrollableNode);

    static WKCFloatRect transformedRect(const WKC::Node* node, const WKCFloatRect& rect);
    ...
    */
    void getCurrentFocusPoint(float& x, float& y);
    /**
    ...
    */
    ...
    bool dispatchNavigationEvent(WKC::NavigationEventType type, WKC::Element* target, WKC::Element* relatedTarget, WKC::FocusDirection dir, bool* defaultPrevented);
