...
public:
    StorageNamespaceWKC(WebCore::Page* page, unsigned quota, WebCore::SecurityOrigin* origin, WebCore::StorageType type, WKC::WKCWebViewPrivate* view,PAL::SessionID sessionId)
        : m_page(page)
        , m_quota(quota)
        , m_origin(origin)
        , m_type(type)
    ...
    virtual WTF::Ref<WebCore::StorageArea> storageArea(const WebCore::SecurityOriginData& origin) override
    {
        return adoptRef(*new StorageAreaWKC(origin.securityOrigin(), m_type, m_quota, m_view));
    }

    virtual WTF::Ref<WebCore::StorageNamespace> copy(WebCore::Page& newPage) override
    {
        PlatformPageClient client = newPage.chrome().client().platformPageClient();
        return adoptRef(*new StorageNamespaceWKC(&newPage, m_quota, m_origin, m_type, (WKC::WKCWebViewPrivate *)client, m_sessionId));
    }

    virtual PAL::SessionID sessionID() const override
    {
        return m_sessionId;
    }
    virtual void setSessionIDForTesting(PAL::SessionID sessionId) override
    {
        m_sessionId = sessionId;
    }
private:
...
WTF::Ref<WebCore::StorageNamespace>
StoragenamespaceProviderWKC::createSessionStorageNamespace(WebCore::Page& page, unsigned quota)
{
    return adoptRef(*new StorageNamespaceWKC(&page, quota, nullptr, WebCore::StorageType::Session, m_view, PAL::SessionID::defaultSessionID()));
}

WTF::Ref<WebCore::StorageNamespace>
StoragenamespaceProviderWKC::createLocalStorageNamespace(unsigned quota, PAL::SessionID)
{
    return adoptRef(*new StorageNamespaceWKC(nullptr, quota, nullptr, WebCore::StorageType::Local, m_view, PAL::SessionID::defaultSessionID()));
}

WTF::Ref<WebCore::StorageNamespace>
StoragenamespaceProviderWKC::createTransientLocalStorageNamespace(WebCore::SecurityOrigin& origin, unsigned quota, PAL::SessionID)
{
    return adoptRef(*new StorageNamespaceWKC(nullptr, quota, &origin, WebCore::StorageType::TransientLocal, m_view, PAL::SessionID::defaultSessionID()));
}
