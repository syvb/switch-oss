...
public:
    ...
    virtual void load(const URL& url, const WebCore::ContentType& type, MediaSourcePrivateClient* client) override final;
#endif
    ...
    virtual void setPageIsVisible(bool) override final;
    ...
    virtual DestinationColorSpace colorSpace() override final;

    virtual void setPreload(MediaPlayer::Preload) override final;
    ...
    virtual void setLoop(bool) override final;
    ...
    virtual std::optional<VideoPlaybackQualityMetrics> videoPlaybackQualityMetrics() override final;
    ...
    // downloader
    virtual void didReceiveData(ResourceHandle*, const SharedBuffer&, int /*encodedDataLength*/) override final;
...
private:
    ...
    static void onCurlDebugProc(void* s, void* h, int t, const char* d, unsigned int u) { return ((MediaPlayerPrivateWKC *)s)->onCurlDebug(h, t, d, u); }
#if ENABLE(ENCRYPTED_MEDIA)
    ...
    void onCurlDebug(void* handle, int type, const char* data, unsigned int size);
    void didSendRequest(Frame* frame, void* handle, const char* data, unsigned int size);
    void didReceiveResponse(Frame* frame, void* handle, const char* data, unsigned int size);
    void didReceiveData(Frame* frame, void* handle, const char* data, unsigned int size);
    void didFinishLoading(Frame* frame, void* handle);
    void didFailLoading(Frame* frame, void* handle, const ResourceError& error);
#if ENABLE(ENCRYPTED_MEDIA)
    ...
    // network
    static int notifyPlayerStreamOpenURIProc(void* s, const char* u, const char* m, const WKCMediaPlayerStreamHeaderInfo* hs, int l, const WKCMediaPlayerStreamStateProcs* p) { return ((MediaPlayerPrivateWKC *)s)->notifyPlayerStreamOpenURI(u,m,hs,l,p); }
    ...
    int notifyPlayerStreamOpenURI(const char* uri, const char* method, const WKCMediaPlayerStreamHeaderInfo* headers, int len, const WKCMediaPlayerStreamStateProcs* procs);
    int notifyPlayerStreamClose(int id);
    int notifyPlayerStreamCancel(int id);

    typedef struct httpConnection_ {
        int m_id;
        RefPtr<ResourceHandle> m_connection;
        WKCMediaPlayerStreamStateProcs m_procs;
    } httpConnection;
...
private:
    ...
    struct NetworkInfo {
        ...
        int64_t m_receivedLength{ 0 };
        unsigned int m_requestHeaderSize{ 0 };
        unsigned int m_requestBodySize{ 0 };
        unsigned int m_responseHeaderSize{ 0 };
    };
    WTF::HashMap<void*, NetworkInfo> m_networkInfo;
};
