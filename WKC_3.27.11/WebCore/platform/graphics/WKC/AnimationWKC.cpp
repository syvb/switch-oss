...
// copy from WebCore/platform/graphics/texmap/TextureMapperAnimation.cpp
...
AnimationWKC::AnimationWKC(const String& name, const KeyframeValueList& keyframes, const FloatSize& boxSize, const Animation& animation, bool listsMatch, MonotonicTime startTime, Seconds pauseTime, AnimationState state)
...
AnimationWKC::AnimationWKC(const AnimationWKC& other)
...
void AnimationWKC::apply(ApplicationResult& applicationResults, MonotonicTime time)
{
    if (!isActive())
        return;

    Seconds totalRunningTime = computeTotalRunningTime(time);
    if (totalRunningTime < 0_s) {
        return;
    }
    ...
    if (m_keyframes.size() == 2) {
        ...
        normalizedValue = timingFunction.transformProgress(normalizedValue, m_animation->duration());
...
void AnimationWKC::pause(Seconds time)
{
    m_state = AnimationState::Paused;
    m_pauseTime = time;
}

void AnimationWKC::resume()
{
...
Seconds AnimationWKC::computeTotalRunningTime(MonotonicTime time)
{
...
bool AnimationWKC::isActive() const
{
    return m_state != AnimationState::Stopped || m_animation->fillsForwards();
}

void AnimationWKC::applyInternal(ApplicationResult& applicationResults, const AnimationValue& from, const AnimationValue& to, float progress)
{
...
void AnimationsWKC::add(const AnimationWKC& animation)
{
...
void AnimationsWKC::remove(const String& name)
{
    m_animations.removeAllMatching([&name] (const AnimationWKC& animation) {
        return animation.name() == name;
    });
}

void AnimationsWKC::remove(const String& name, AnimatedPropertyID property)
{
    m_animations.removeAllMatching([&name, property] (const AnimationWKC& animation) {
        return animation.name() == name && animation.keyframes().property() == property;
    });
}

void AnimationsWKC::pause(const String& name, Seconds offset)
{
...
void AnimationsWKC::suspend(MonotonicTime time)
{
...
void AnimationsWKC::resume()
{
    for (auto& animation : m_animations)
        animation.resume();
}

void AnimationsWKC::apply(AnimationWKC::ApplicationResult& applicationResults, MonotonicTime time)
{
    for (auto& animation : m_animations)
        animation.apply(applicationResults, time);
}

bool AnimationsWKC::hasActiveAnimationsOfType(AnimatedPropertyID type) const
{
    return std::any_of(m_animations.begin(), m_animations.end(),
        [&type](const AnimationWKC& animation) { return animation.isActive() && animation.keyframes().property() == type; });
}

bool AnimationsWKC::hasRunningAnimations() const
{
    return std::any_of(m_animations.begin(), m_animations.end(),
        [](const AnimationWKC& animation) { return animation.state() == AnimationWKC::AnimationState::Playing; });
}

AnimationsWKC AnimationsWKC::getActiveAnimations() const
{
    AnimationsWKC active;
    for (auto& animation : m_animations) {
