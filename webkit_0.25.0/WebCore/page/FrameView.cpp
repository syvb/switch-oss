...
#if PLATFORM(WKC)
void FrameView::queuePostLayoutCallback(WTF::Function<void()> callback)
{
    m_postLayoutCallbackQueue.append(WTF::move(callback));
#else
...
#if PLATFORM(WKC)
    const auto queue = WTF::move(m_postLayoutCallbackQueue);
#else
