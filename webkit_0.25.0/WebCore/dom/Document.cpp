 ...
 * Copyright (C) 2013-2020 ACCESS CO., LTD. All rights reserved.
 *
...
#else
WKC_DEFINE_GLOBAL_CLASS_OBJ(uint64_t, Document, s_globalTreeVersion, 0);
#endif
...
HashSet<Document*>& Document::allDocuments()
{
    ...
    return documents;
#else
    WKC_DEFINE_STATIC_PTR(HashSet<Document*>*, documents, 0);
    if (!documents)
...
#else
    return std::chrono::milliseconds(elapsedTime.get().count());
#endif
}
...
#else
    WKC_DEFINE_STATIC_PTR(JSC::Yarr::RegularExpression*, youtubePattern, 0);
    if (!youtubePattern)
        youtubePattern = new JSC::Yarr::RegularExpression("(^|\\.)youtube.(com|co.uk|[a-z]{2})$", TextCaseInsensitive);

    ASSERT(youtubePattern->isValid());
    return youtubePattern->match(host);
#endif
}
