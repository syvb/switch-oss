...
#if PLATFORM(WKC)
template<> inline std::pair<WebCore::RenderLayer**, ptrdiff_t>
std::get_temporary_buffer<WebCore::RenderLayer*>(ptrdiff_t count) _NOEXCEPT
{
    WebCore::RenderLayer** p = (WebCore::RenderLayer**)WTF::fastMalloc(sizeof(WebCore::RenderLayer*) * count);
    return (std::pair<WebCore::RenderLayer**, ptrdiff_t>(p, count));
}
template<> inline void
std::return_temporary_buffer<WebCore::RenderLayer*>(WebCore::RenderLayer** buf) _NOEXCEPT
{
    WTF::fastFree(buf);
}
#endif
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_PTR(Vector<RenderLayer*>*, gValidLayers, 0);
#endif
...
#if PLATFORM(WKC)
    if (!gValidLayers)
        gValidLayers = new Vector<RenderLayer*>();
    gValidLayers->append(this);
#endif
}
...
#if PLATFORM(WKC)
    if (gValidLayers) {
        size_t pos = gValidLayers->find(this);
        if (pos != notFound)
            gValidLayers->remove(pos);
    }
#endif
...
#if PLATFORM(WKC)
                if (!gValidLayers->contains(layer))
                    continue;
#endif
...
RenderLayer* RenderLayer::stackingContainer() const
{
    ...
    while (layer && !layer->renderer().isRoot() && !layer->isStackingContainer())
#else
...
void RenderLayer::drawPlatformResizerImage(GraphicsContext* context, const LayoutRect& resizerCornerRect)
{
    ...
    if (renderer().document().deviceScaleFactor() >= 2) {
...
#else
        WKC_DEFINE_STATIC_PTR(Image*, resizeCornerImageHiRes, nullptr);
        if (!resizeCornerImageHiRes)
            resizeCornerImageHiRes = Image::loadPlatformResource("extAreaResizeCorner@2x").leakRef();
#endif
        ...
        cornerResizerSize = resizeCornerImage->size();
#else
        WKC_DEFINE_STATIC_PTR(Image*, resizeCornerImageLoRes, nullptr);
        if (!resizeCornerImageLoRes)
            resizeCornerImageLoRes = Image::loadPlatformResource("textAreaResizeCorner").leakRef();
        resizeCornerImage = resizeCornerImageLoRes;
        cornerResizerSize = resizeCornerImage->size();
        cornerResizerSize.scale(Image::platformResourceScale());
#endif
    }
...
void RenderLayer::paint(GraphicsContext* context, const LayoutRect& damageRect, const LayoutSize& subpixelAccumulation, PaintBehavior paintBehavior, RenderObject* subtreePaintRoot, PaintLayerFlags paintFlags, SecurityOriginPaintPolicy paintPolicy)
{
#if PLATFORM(WKC)
    CRASH_IF_STACK_OVERFLOW(WKC_STACK_MARGIN_DEFAULT);
#endif
    ...
    LayerPaintingInfo paintingInfo(this, enclosingIntRect(damageRect), paintBehavior, subpixelAccumulation, subtreePaintRoot, &overlapTestRequests, paintPolicy == SecurityOriginPaintPolicy::AccessibleOriginOnly);
...
void RenderLayer::paintLayerByApplyingTransform(GraphicsContext* context, const LayerPaintingInfo& paintingInfo, PaintLayerFlags paintFlags, const LayoutSize& translationOffset)
{
    ...
    transformedPaintingInfo.paintDirtyRect = LayoutRect(encloseRectToDevicePixels(transform.inverse().mapRect(paintingInfo.paintDirtyRect), deviceScaleFactor));
    transformedPaintingInfo.subpixelAccumulation = adjustedSubpixelAccumulation;
...
#if PLATFORM(WKC)
            if (!gValidLayers->contains(curLayer))
                continue;
#endif
