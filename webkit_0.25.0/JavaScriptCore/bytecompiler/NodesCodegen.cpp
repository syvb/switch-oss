...
#if PLATFORM(WKC)
            generator.emitEnumeration(spread, spread->expression(), WTF::move(spreader));
#else
...
#else
    return m_emitter(this, generator, dst);
#endif
}
...
#if PLATFORM(WKC)
                generator.emitEnumeration(this, spread->expression(), WTF::move(extractor));
#else
...
void ForInNode::emitLoopHeader(BytecodeGenerator& generator, RegisterID* propertyName)
{
    if (m_lexpr->isResolveNode()) {
        ...
        if (RegisterID* local = var.local()) {
            ...
            RefPtr<RegisterID> scope = generator.emitResolveScope(nullptr, var);
            generator.emitExpressionInfo(divot(), divotStart(), divotEnd());
            generator.emitPutToScope(scope.get(), var, propertyName, generator.isStrictMode() ? ThrowIfNotFound : DoNotThrowIfNotFound);
            if (generator.vm()->typeProfiler())
    ...
    auto extractor = [this, dst](BytecodeGenerator& generator, RegisterID* value)
    {
        if (m_lexpr->isResolveNode()) {
            ...
            if (RegisterID* local = var.local()) {
                ...
                RefPtr<RegisterID> scope = generator.emitResolveScope(nullptr, var);
                generator.emitExpressionInfo(divot(), divotStart(), divotEnd());
                generator.emitPutToScope(scope.get(), var, value, generator.isStrictMode() ? ThrowIfNotFound : DoNotThrowIfNotFound);
                if (generator.vm()->typeProfiler())
...
#if PLATFORM(WKC)
    generator.emitEnumeration(this, m_expr, WTF::move(extractor));
#else
