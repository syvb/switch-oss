...
#if !defined(HAS_ATOMIC_OPS) && defined(_WIN32) && !defined(WKC_CAIRO_CUSTOMIZE)
#include <windows.h>
...
#define _cairo_status_set_error(status, err) do { \
    ...
    /* cairo_status_t != long long. \
     * casting from &cairo_status_t to cairo_atomic_int_t* may not result in 0 in upper 4 bytes. */ \
