...
WKC_PEER_API void wkcMemoryNotifyStackOverflowPeer(size_t margin, size_t consumption, void* currentStackTop, const char* file, int line, const char* function);
WKC_PEER_API bool wkcMemoryIsStackOverflowPeer(size_t margin, size_t* consumption, void** currentStackTop);
...
# define CAIRO_CRASH_IF_STACK_OVERFLOW(margin) do { \
    size_t __consumption; \
    void* __stack_top; \
    if (wkcMemoryIsStackOverflowPeer((margin), &__consumption, &__stack_top)) { \
        wkcMemoryNotifyStackOverflowPeer((margin), __consumption, __stack_top, __FILE__, __LINE__, __FUNCTION__); \
    } \
} while (FALSE)
# define CAIRO_CRASH() do { wkcMemorySetCrashReasonPeer(__FILE__,__LINE__,__FUNCTION__,"CRASH()"); wkcMemoryNotifyCrashPeer(); } while (FALSE)
# define CAIRO_COMPILATION 1
#endif
...
#ifdef WKC_CAIRO_CUSTOMIZE
    ,
    CAIRO_SURFACE_TYPES = 0x7fffffff
#endif
} cairo_surface_type_t;
