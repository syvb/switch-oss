...
#else
WKC_DEFINE_GLOBAL_CLASS_OBJ(MonotonicTime, FrameView, sCurrentPaintTimeStamp, MonotonicTime());
#endif
...
void FrameView::applyOverflowToViewport(const RenderElement& renderer, ScrollbarMode& hMode, ScrollbarMode& vMode)
{
    ...
    if (Page* page = frame().page())
        page->chrome().client().dispatchApplyOverflowToViewport(frame(), (overflowX == Overflow::Hidden), (overflowY == Overflow::Hidden));
#endif
}
...
#if PLATFORM(WKC)
    if (!frame().settings().visualViewportEnabled())
        return LayoutRect(scrollPositionForFixedPosition(), baseLayoutViewportSize());
#endif
    if (m_layoutViewportOverrideRect)
...
FloatRect FrameView::absoluteToLayoutViewportRect(FloatRect rect) const
{
    ...
    rect.moveBy(-layoutViewportRect().location());
#else
    rect.moveBy(-scrollPositionForFixedPosition());
#endif
    return rect;
}
