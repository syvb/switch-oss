...
#if PLATFORM(WKC)
    else if (equalLettersIgnoringASCIICase(name(), "focus-ring-visibility"))
        document().processFocusRingVisibility(contentValue);
    else if (equalLettersIgnoringASCIICase(name(), "initial-autofocus"))
        document().processInitialAutofocus(contentValue);
#endif
