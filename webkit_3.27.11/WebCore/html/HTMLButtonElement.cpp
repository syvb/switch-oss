...
#if PLATFORM(WKC)
            if (submit.isNull())
                submit.construct("submit", AtomString::ConstructFromLiteral);
#endif
            return submit;
        }
        case BUTTON: {
...
#if PLATFORM(WKC)
            if (button.isNull())
                button.construct("button", AtomString::ConstructFromLiteral);
#endif
            return button;
        }
        case RESET: {
...
#if PLATFORM(WKC)
            if (reset.isNull())
                reset.construct("reset", AtomString::ConstructFromLiteral);
#endif
            return reset;
        }
    }
