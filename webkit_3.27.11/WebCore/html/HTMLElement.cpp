...
#else
    static LazyNeverDestroyed<EventHandlerNameMap> map;
    if (map.isNull())
        map.construct([](){
#endif
...
#if PLATFORM(WKC)
    if (trueValue.isNull())
        trueValue.construct("true", AtomString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC)
    if (falseValue.isNull())
        falseValue.construct("false", AtomString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC)
    if (plaintextOnlyValue.isNull())
        plaintextOnlyValue.construct("plaintext-only", AtomString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC)
    if (onName.isNull())
        onName.construct("on", AtomString::ConstructFromLiteral);
#endif
    static MainThreadNeverDestroyed<const AtomString> offName("off", AtomString::ConstructFromLiteral);
#if PLATFORM(WKC)
    if (offName.isNull())
        offName.construct("off", AtomString::ConstructFromLiteral);
#endif
