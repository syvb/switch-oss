...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_CLASS_OBJ_ZERO(bool, Document, hasEverCreatedAnAXObjectCache);
#else
...
#else
WKC_DEFINE_GLOBAL_CLASS_OBJ_ZERO(uint64_t, Document, s_globalTreeVersion);
#endif
...
#else
    static LazyNeverDestroyed<uint64_t> owner;
    if (owner.isNull())
        owner.construct(cryptographicallyRandomNumber());
    return reinterpret_cast<const void*>(owner.get());
#endif
}
...
#else
    static LazyNeverDestroyed<Logger*> logger;
    if (logger.isNull())
        logger.construct(nullptr);
#endif
    return logger;
}
...
#if PLATFORM(WKC)
    if (annotationXmlLocalName.isNull())
        annotationXmlLocalName.construct("annotation-xml", AtomString::ConstructFromLiteral);
#endif
#endif
    static MainThreadNeverDestroyed<const AtomString> colorProfileLocalName("color-profile", AtomString::ConstructFromLiteral);
#if PLATFORM(WKC)
    if (colorProfileLocalName.isNull())
        colorProfileLocalName.construct("color-profile", AtomString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC) && ENABLE(TOUCH_EVENTS)
...
#if PLATFORM(WKC)
void Document::processFocusRingVisibility(const String& value)
{
    if (Page* page = this->page())
        page->chrome().client().dispatchMetaFocusRingVisibilityDidChange(value);
}

void Document::processInitialAutofocus(const String& value)
{
    if (Page* page = this->page())
        page->chrome().client().dispatchMetaInitialAutofocusDidChange(value);
}
#endif
...
{
#if (PLATFORM(IOS_FAMILY) || PLATFORM(WKC)) && ENABLE(TOUCH_EVENTS)
    setTouchEventRegionsNeedUpdate();
#endif
...
#if PLATFORM(WKC) && ENABLE(TOUCH_EVENTS)
void Document::updateTouchEventRegions()
{
    if (!m_touchEventRegionsNeedUpdate)
        return;

    if (view())
        updateTouchEventRegionsForFrame();

    m_touchEventRegionsNeedUpdate = false;
}

void Document::updateTouchEventRegionsForFrame()
{
    EventTrackingRegions eventTrackingRegions;

    // FIXME: if we've already accounted for this subframe as a scrollable area, we can avoid recursing into it here.
    for (auto* subframe = m_frame->tree().firstChild(); subframe && subframe->document() && subframe->view(); subframe = subframe->tree().nextSibling()) {
        subframe->document()->updateTouchEventRegionsForFrame();
        EventTrackingRegions subframeRegion = subframe->document()->eventTrackingRegions();
        // Map from the frame document to our document.
        IntPoint offset = subframe->view()->contentsToContainingViewContents(IntPoint());
    ...
    if (!touchEventTargets())
        return;

    for (auto& keyValuePair : *touchEventTargets()) {
        if (RefPtr node = keyValuePair.key) {
            auto targetRegionFixedPair = absoluteEventRegionForNode(*node);
            Region targetRegion = targetRegionFixedPair.first;
            if (targetRegionFixedPair.second) {
                // FIXME: need to handle position:sticky here too.
                LayoutRect inflatedBounds = view()->fixedScrollableAreaBoundsInflatedForScrolling(LayoutRect(targetRegion.bounds()));
                targetRegion = Region(enclosingIntRect(inflatedBounds));
            }

            bool isStartActive = node->ensureEventTargetData().eventListenerMap.containsActive(eventNames().touchstartEvent);
            if (isStartActive)
                eventTrackingRegions.uniteSynchronousRegion(eventNames().touchstartEvent, targetRegion);

            bool isMoveActive = node->ensureEventTargetData().eventListenerMap.containsActive(eventNames().touchmoveEvent);
            if (isMoveActive)
                eventTrackingRegions.uniteSynchronousRegion(eventNames().touchmoveEvent, targetRegion);

            if (!isStartActive && !isMoveActive)
                eventTrackingRegions.asynchronousDispatchRegion.unite(targetRegion);
        }
    }
...
{
    DesignMode mode = equalLettersIgnoringASCIICase(value, "on") ? DesignMode::On : DesignMode::Off;
    setDesignMode(mode);
}
...
#if PLATFORM(WKC)
    if (quickLookCSP.isNull())
        quickLookCSP.construct(makeString("default-src ", QLPreviewProtocol, ": 'unsafe-inline'; base-uri 'none'; sandbox allow-same-origin allow-scripts"));
#endif
...
static MessageSource messageSourceForWTFLogChannel(const WTFLogChannel& channel)
{
    ...
    if (mediaChannel.isNull())
        mediaChannel.construct(MAKE_STATIC_STRING_IMPL("media"));
#endif
    static const NeverDestroyed<String> webrtcChannel = MAKE_STATIC_STRING_IMPL("webrtc");
#if PLATFORM(WKC)
    if (webrtcChannel.isNull())
        webrtcChannel.construct(MAKE_STATIC_STRING_IMPL("webrtc"));
#endif
...
#endif
#if PLATFORM(WKC)
    if (mediaSourceChannel.isNull())
        mediaSourceChannel.construct(MAKE_STATIC_STRING_IMPL("mediasource"));
#endif
