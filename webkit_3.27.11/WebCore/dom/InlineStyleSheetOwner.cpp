...
#if PLATFORM(WKC)
    auto& baseURL = shadowRoot && shadowRoot->mode() == ShadowRootMode::UserAgent ? aboutBlankURL() : element.document().baseURL().isNull() ? aboutBlankURL() : element.document().baseURL();
#else
...
#if PLATFORM(WKC)
    if (cssContentType.isNull())
        cssContentType.construct("text/css", AtomString::ConstructFromLiteral);
#endif
