...
public:
...
#if PLATFORM(WKC)
        if (!m_png || !m_info) {
            wkcMemoryNotifyNoMemoryPeer(1296 + 360 /* approximate size: sizeof(png_struct) + sizeof(png_info) */);
        }
#endif
#if ENABLE(APNG)
    ...
    bool decode(const SharedBuffer& data, bool sizeOnly, unsigned haltAtFrame)
    {
        ...
        while (m_readOffset < data.size()) {
            auto bytesToSkip = m_readOffset;
            auto bytesToUse = data.size() - bytesToSkip;
            if (bytesToUse > segmentSize)
                bytesToUse = segmentSize;
#else
        auto bytesToSkip = m_readOffset;
        auto bytesToUse = data.size() - bytesToSkip;
#endif
    ...
    {
        auto bufferSize = sizeof(png_byte) * size;
        ...
        m_interlaceBuffer = makeUniqueArray<png_byte>(size);
    ...
    if (buffer.isInvalid()) {
        ...
        if (m_currentFrame) {
            if (!initFrameBuffer(m_currentFrame)) {
                buffer.setDecodingStatus(DecodingStatus::Invalid);
                longjmp(JMPBUF(png), 1);
                return;
            }
        }
...
#if PLATFORM(WKC)
        if (m_frameBufferCache.capacity() < m_frameCount)
        {
            constexpr size_t elemSize = sizeof(decltype(m_frameBufferCache[0]));
            static_assert(elemSize < std::numeric_limits<size_t>::max() / PNG_UINT_31_MAX);
            size_t newBufferSize = m_frameCount * elemSize;
            ...
            if (!m_frameBufferCache.tryReserveCapacity(newBufferSize)) {
                fallbackNotAnimated();
                wkcMemoryNotifyMemoryAllocationErrorPeer(newBufferSize, WKC_MEMORYALLOC_TYPE_IMAGE);
                return;
            }
...
void PNGImageDecoder::init()
{
    ...
    m_sequenceNumber = 0;
#if PLATFORM(WKC)
    for (auto& cache : m_frameBufferCache)
        cache.clear();
#endif
}
...
#if PLATFORM(WKC)
    clearBeforeFrame = std::min(clearBeforeFrame, static_cast<size_t>(m_currentFrame - 1));
#endif
...
#if PLATFORM(WKC)
void PNGImageDecoder::clearFrameBufferCache()
{
    for (auto& frame : m_frameBufferCache) {
        frame.clear();
    }
    m_reader = nullptr;
}
...
#else
bool PNGImageDecoder::initFrameBuffer(size_t frameIndex)
#endif
{
#if !PLATFORM(WKC)
