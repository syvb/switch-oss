...
#else
    static float getTextWidth(const FontPlatformData& in_font, int in_flags, const UChar* in_str, int in_len, float* out_clip_width);
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY(CodePath, s_codePath);
    static UChar* fixedU16Text(const FontPlatformData& in_font, const UChar* str, int& len, bool& needfree);
#endif
