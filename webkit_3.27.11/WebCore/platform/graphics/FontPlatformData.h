...
#if PLATFORM(WKC)
    WKC::WKCFontInfo* font() const { return m_font; }
#endif
...
#else
    bool isSupplemental() const { return !!m_supplementaryCharBits; }
    UChar32 supplementaryCharBits() const { return m_supplementaryCharBits; }
    void setSupplementaryCharBits(UChar32 supplementaryCharBits) const { m_supplementaryCharBits = supplementaryCharBits; }
...
#if PLATFORM(WKC)
    void platformDataAssign(const FontPlatformData&);
#endif
...
#elif PLATFORM(WKC)
    WKC::WKCFontInfo* m_font{ nullptr };
#endif
...
#if PLATFORM(WKC)
    bool m_isEmoji { false };
    mutable UChar32 m_supplementaryCharBits { 0 };
#endif
