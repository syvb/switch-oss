...
#else
    static LazyNeverDestroyed<cairo_user_data_key_t> s_surfaceDataKey;
    if (s_surfaceDataKey.isNull())
        s_surfaceDataKey.construct();
#endif
...
#else
    cairo_surface_set_user_data(surface.get(), &s_surfaceDataKey.get(), surfaceData, [](void* data) { fastFree(data); });
#endif
