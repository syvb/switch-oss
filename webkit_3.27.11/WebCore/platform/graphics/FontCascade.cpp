...
#else
    static LazyNeverDestroyed<MemoryCompactLookupOnlyRobinHoodHashSet<AtomString>> set;
    if (set.isNull())
        set.construct([](){
#endif
...
#else
WKC_DEFINE_GLOBAL_CLASS_OBJ(FontCascade::CodePath, FontCascade, s_codePath, CodePath::Auto)
#endif
...
#else
WKC_DEFINE_GLOBAL_BOOL(shouldUseFontSmoothing, true);
#endif
...
#else
        const UChar32 nextCharacter = glyphBuffer.glyphAt(nextGlyph) | nextFontData.platformData().supplementaryCharBits();
        if (&nextFontData != fontData &&
#if defined(WKC_USE_ICU)
            !(U_GET_GC_MASK(nextCharacter) & U_GC_MN_MASK)) {
#else
            wkcUnicodeCategoryPeer(nextCharacter) != WKC_UNICODE_CATEGORY_MARKNONSPACING) {
#endif
#endif
