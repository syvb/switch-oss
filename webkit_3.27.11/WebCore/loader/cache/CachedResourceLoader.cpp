...
bool CachedResourceLoader::checkInsecureContent(CachedResource::Type type, const URL& url) const
{
    ...
    case CachedResource::Type::CSSStyleSheet:
        ...
        if (Frame* frame = this->frame()) {
            ...
            if (&top != frame && top.document() && !MixedContentChecker::canRunInsecureContent(top, top.document()->securityOrigin(), url))
                return false;
        }
        break;
#if ENABLE(VIDEO)
    ...
    case CachedResource::Type::FontResource: {
        ...
        if (Frame* frame = this->frame()) {
            ...
            if (!topFrame.document() || !MixedContentChecker::canDisplayInsecureContent(topFrame, topFrame.document()->securityOrigin(), contentTypeFromResourceType(type), url))
                return false;
        }
        break;
    }
...
{
    if (m_document) {
        if (!m_document->securityOrigin().canDisplay(url)) {
...
#if PLATFORM(WKC)
    if (!equalIgnoringASCIICase(request.resourceRequest().httpMethod(), "GET")
        && !equalIgnoringASCIICase(request.resourceRequest().httpMethod(), "HEAD")
        && !equalIgnoringASCIICase(request.resourceRequest().httpMethod(), "OPTIONS")
        && !equalIgnoringASCIICase(request.resourceRequest().httpMethod(), "TRACE"))
        return true;
    if (resource.response().responseFired() && !isStatusCodeCacheableByDefault(resource.response().httpStatusCode()))
        return true;
#endif
...
void CachedResourceLoader::warnUnusedPreloads()
{
    ...
    for (const auto& resource : *m_preloads) {
        if (resource && resource->isLinkPreload() && resource->preloadResult() == CachedResource::PreloadResult::PreloadNotReferenced) {
