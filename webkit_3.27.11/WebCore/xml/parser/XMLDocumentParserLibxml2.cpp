...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_INT_ZERO(globalDescriptor);
WKC_DEFINE_GLOBAL_TYPE_ZERO(Thread*, libxmlLoaderThread);
#else
...
{
#if PLATFORM(WKC)
    WKC_DEFINE_STATIC_TYPE(xmlEntity, entity, {});
#else
    static xmlEntity entity;
#endif
    if (!entity.type) {
#if PLATFORM(WKC)
        for (int i = 0; i < 9; i++) sharedXHTMLEntityResult[i] = 0;
#endif
