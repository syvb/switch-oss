...
#if PLATFORM(WKC)
    LayoutUnit highValue() const { return m_nextShapeLogicalBottom.value_or(m_nextLogicalBottom.value_or(LayoutUnit::max())); }
#else
...
#else
    if (!rangesIntersect(m_renderer->logicalTopForFloat(floatingObject), m_renderer->logicalBottomForFloat(floatingObject), m_lineTop, m_lineBottom))
        return;
#endif
