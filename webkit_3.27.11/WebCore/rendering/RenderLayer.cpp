...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_TYPE_ZERO(Vector<RenderLayer*>*, gValidLayers);
#endif
...
#else
    WKC_DEFINE_STATIC_TYPE(ScrollingScope, currentScope, 0);
#endif
    return ++currentScope;
}
...
void RenderLayer::scrollRectToVisible(const LayoutRect& absoluteRect, bool insideFixed, const ScrollRectToVisibleOptions& options)
{
    ...
    if (allowsCurrentScroll()) {
        ...
        ASSERT(box);
#if PLATFORM(WKC)
        if (options.revealMode == SelectionRevealMode::RevealUpToFrameOrScrollsOverflow) {
            if (box->scrollsOverflow() && box->scrollsOverflow())
                return;
        }
#endif
