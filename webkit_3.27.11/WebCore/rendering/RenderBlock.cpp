...
#else
WKC_DEFINE_GLOBAL_TYPE_ZERO(TrackedDescendantsMap*, percentHeightDescendantsMap);
WKC_DEFINE_GLOBAL_TYPE_ZERO(TrackedContainerMap*, percentHeightContainerMap);
#endif
...
#if PLATFORM(WKC)
    if (mapForPositionedDescendants.isNull())
        mapForPositionedDescendants.construct();
#endif
...
#if PLATFORM(WKC)
    if (delayedUpdatedScrollInfoStack.isNull())
        delayedUpdatedScrollInfoStack.construct();
#endif
...
#else
WKC_DEFINE_GLOBAL_TYPE_ZERO(RenderBlockRareDataMap*, gRareDataMap);
#endif
...
#if PLATFORM(WKC)
void RenderBlock::addFocusRingRectsForInlineChildren(Vector<LayoutRect>&, const LayoutPoint&, const RenderLayerModelObject*, bool)
#else
...
#if PLATFORM(WKC)
void RenderBlock::addFocusRingRects(Vector<LayoutRect>& rects, const LayoutPoint& additionalOffset, const RenderLayerModelObject* paintContainer, bool useTransform)
#else
...
#endif
{
    ...
    if (inlineContinuation) {
        ...
        if (useTransform) {
            ...
            if (!rect.isEmpty())
                rects.append(snapRectToDevicePixels(LayoutRect(localToContainerQuad(rect, paintContainer).boundingBox()), document().deviceScaleFactor()));
        } else {
            LayoutRect rect(additionalOffset.x(), additionalOffset.y() - topMargin, width(), height() + topMargin + bottomMargin);
            if (!rect.isEmpty())
                rects.append(rect);
...
#else
    {
        if (useTransform)
            // Add transformed rect.
            rects.append(snapRectToDevicePixels(LayoutRect(localToContainerQuad(FloatRect(0, 0, width(), height()), paintContainer).boundingBox()), document().deviceScaleFactor()));
        else
            rects.append(LayoutRect(additionalOffset, size()));
    }
#endif
...
#else
    bool isButton = element() && element()->hasTagName(buttonTag);
    if (!hasNonVisibleOverflow() && !hasControlClip() && !isButton) {
#endif
        if (childrenInline())
#if !PLATFORM(WKC)
            addFocusRingRectsForInlineChildren(rects, additionalOffset, paintContainer);
#else
            addFocusRingRectsForInlineChildren(rects, additionalOffset, paintContainer, useTransform);
#endif
        ...
        for (auto& box : childrenOfType<RenderBox>(*this)) {
...
#else
            if (useTransform)
                // get transformed position always.
                pos = box.localToContainerPoint(FloatPoint(), paintContainer);
            else {
                // FIXME: This doesn't work correctly with transforms.
                if (box.layer())
            ...
            box.addFocusRingRects(rects, flooredLayoutPoint(pos), paintContainer, useTransform);
#endif
        }
    }
...
#else
        inlineContinuation->addFocusRingRects(rects, flooredLayoutPoint(LayoutPoint(additionalOffset + inlineContinuation->containingBlock()->location() - location())), paintContainer, useTransform);
#endif
}
