...
#if PLATFORM(WKC)
    void addFocusRingRects(Vector<LayoutRect>&, const LayoutPoint& additionalOffset, const RenderLayerModelObject* paintContainer = 0, bool useTransform = true) override;
    virtual void addFocusRingRectsForInlineChildren(Vector<LayoutRect>&, const LayoutPoint& additionalOffset, const RenderLayerModelObject* paintContainer, bool useTransform);
#else
...
#else
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY_ZERO(bool, s_canPropagateFloatIntoSibling);
#endif
};
