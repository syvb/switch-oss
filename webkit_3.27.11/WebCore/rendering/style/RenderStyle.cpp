...
#if PLATFORM(WKC)
    if (style.isNull())
        style.construct(RenderStyle { CreateDefaultStyle });
#endif
    return style;
}
...
#if PLATFORM(WKC)
    if (hyphenMinusString.isNull())
        hyphenMinusString.construct(&hyphenMinus, 1);
#endif
    static MainThreadNeverDestroyed<const AtomString> hyphenString(&hyphen, 1);
#if PLATFORM(WKC)
    if (hyphenString.isNull())
        hyphenString.construct(&hyphen, 1);
#endif
...
#if PLATFORM(WKC)
        if (filledDotString.isNull())
            filledDotString.construct(&bullet, 1);
#endif
        static MainThreadNeverDestroyed<const AtomString> openDotString(&whiteBullet, 1);
#if PLATFORM(WKC)
        if (openDotString.isNull())
            openDotString.construct(&whiteBullet, 1);
#endif
...
#if PLATFORM(WKC)
        if (filledCircleString.isNull())
            filledCircleString.construct(&blackCircle, 1);
#endif
        static MainThreadNeverDestroyed<const AtomString> openCircleString(&whiteCircle, 1);
#if PLATFORM(WKC)
        if (openCircleString.isNull())
            openCircleString.construct(&whiteCircle, 1);
#endif
...
#if PLATFORM(WKC)
        if (filledDoubleCircleString.isNull())
            filledDoubleCircleString.construct(&fisheye, 1);
#endif
        static MainThreadNeverDestroyed<const AtomString> openDoubleCircleString(&bullseye, 1);
#if PLATFORM(WKC)
        if (openDoubleCircleString.isNull())
            openDoubleCircleString.construct(&bullseye, 1);
#endif
...
#if PLATFORM(WKC)
        if (filledTriangleString.isNull())
            filledTriangleString.construct(&blackUpPointingTriangle, 1);
#endif
        static MainThreadNeverDestroyed<const AtomString> openTriangleString(&whiteUpPointingTriangle, 1);
#if PLATFORM(WKC)
        if (openTriangleString.isNull())
            openTriangleString.construct(&whiteUpPointingTriangle, 1);
#endif
...
#if PLATFORM(WKC)
        if (filledSesameString.isNull())
            filledSesameString.construct(&sesameDot, 1);
#endif
        static MainThreadNeverDestroyed<const AtomString> openSesameString(&whiteSesameDot, 1);
#if PLATFORM(WKC)
        if (openSesameString.isNull())
            openSesameString.construct(&whiteSesameDot, 1);
#endif
