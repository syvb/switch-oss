...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_TYPE_ZERO(OverrideSizeMap*, gOverridingLogicalHeightMap);
WKC_DEFINE_GLOBAL_TYPE_ZERO(OverrideSizeMap*, gOverridingLogicalWidthMap);
#else
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_TYPE_ZERO(OverridingLengthMap*, gOverridingLogicalHeightLengthMap);
WKC_DEFINE_GLOBAL_TYPE_ZERO(OverridingLengthMap*, gOverridingLogicalWidthLengthMap);
#else
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_TYPE_ZERO(OverrideOptionalSizeMap*, gOverridingContainingBlockContentLogicalHeightMap);
WKC_DEFINE_GLOBAL_TYPE_ZERO(OverrideOptionalSizeMap*, gOverridingContainingBlockContentLogicalWidthMap);
#else
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_CLASS_OBJ_ZERO(bool, RenderBox, s_hadNonVisibleOverflow);
#else
...
#else
void RenderBox::addFocusRingRects(Vector<LayoutRect>& rects, const LayoutPoint& additionalOffset, const RenderLayerModelObject* paintContainer, bool useTransform)
{
    if (useTransform) {
    ...
    if (is<RenderBox>(containingBlock)) {
        if (style().position() == PositionType::Fixed) {
            if ((containingBlock.hasTransform() && containingBlock.isRenderBlock()) || containingBlock.isSVGForeignObject()) {
                return adjustForAbsoluteZoom(downcast<const RenderBox>(containingBlock).clientLogicalWidth(), containingBlock.style());
            } else if (FrameView* view = document().view()) {
                if (RenderView* renderView = document().renderView())
                    return adjustForAbsoluteZoom(view->visibleContentRectIncludingScrollbars().width(), renderView->style()); // but not view->layoutWidth() to enforce non-fixed layout
            }
        }
    }
#endif
...
LayoutUnit RenderBox::containingBlockLogicalHeightForPositioned(const RenderBoxModelObject& containingBlock, bool checkForPerpendicularWritingMode) const
{
    ...
    if (is<RenderBox>(containingBlock)) {
        if (style().position() == PositionType::Fixed) {
            if ((containingBlock.hasTransform() && containingBlock.isRenderBlock()) || containingBlock.isSVGForeignObject()) {
                return adjustForAbsoluteZoom(downcast<const RenderBox>(containingBlock).clientLogicalHeight(), containingBlock.style());
            } else if (FrameView* view = document().view()) {
                if (RenderView* renderView = document().renderView())
                    return adjustForAbsoluteZoom(view->visibleContentRectIncludingScrollbars().height(), renderView->style()); // but not view->layoutWidth() to enforce non-fixed layout
            }
        }
    }
#endif
