...
sub GenerateEnumerationImplementationContent
{
    ...
    $result .= "    };\n";
#WKC_CUSTOMIZE BEGIN
    $result .= "#if PLATFORM(WKC)\n";
    foreach my $value (@{$enumeration->values}) {
        my $enumerationValueName = GetEnumerationValueName($value);
        $result .= "    if (values[static_cast<size_t>(${className}::$enumerationValueName)].isNull())\n";
        if ($value eq "") {
            $result .= "        values[static_cast<size_t>(${className}::$enumerationValueName)].construct(emptyString());\n";
        } else {
            $result .= "        values[static_cast<size_t>(${className}::$enumerationValueName)].construct(MAKE_STATIC_STRING_IMPL(\"$value\"));\n";
        }
    }
    $result .= "#endif\n";
#WKC_CUSTOMIZE END
...
#WKC_CUSTOMIZE BEGIN
        push(@headerContent, "#if PLATFORM(WKC)\n");
        push(@headerContent, "    if (owner.isNull())\n");
        push(@headerContent, "        owner.construct();\n");
        push(@headerContent, "#endif\n");
#WKC_CUSTOMIZE END
