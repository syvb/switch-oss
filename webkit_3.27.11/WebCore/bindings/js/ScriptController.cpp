...
{
#if !PLATFORM(IOS_FAMILY) && !PLATFORM(WKC)
    JSC::initialize();
...
#if PLATFORM(WKC)
ValueOrException ScriptController::evaluateInWorld(const ScriptSourceCode& sourceCode, DOMWrapperWorld& world, Node* thisObject)
#else
...
#if PLATFORM(WKC)
    JSValue thisValue = &proxy;
    if (thisObject) {
        thisValue = jsValueForNode(world, *thisObject);
        if (!thisValue)
            return makeUnexpected(ExceptionDetails { "Cannot convert JSValue for Node"_s });
    }
#endif
...
#if PLATFORM(WKC)
    JSValue returnValue = JSExecState::profiledEvaluate(&globalObject, JSC::ProfilingReason::Other, jsSourceCode, thisValue, evaluationException);
#else
...
#if !PLATFORM(COCOA) && !PLATFORM(WKC)
...
#if PLATFORM(WKC)
JSValue ScriptController::jsValueForNode(DOMWrapperWorld& world, Node& node)
{
    // Can't create JSObjects when JavaScript is disabled
    if (!canExecuteScripts(NotAboutToExecuteScript))
        return {};
    ...
    JSValue jsValue = toJS(globalObj, globalObj, node);
...
#if PLATFORM(WKC)
JSC::JSValue ScriptController::executeScriptIgnoringException(const String& script, bool forceUserGesture, Node* thisObject)
#else
JSC::JSValue ScriptController::executeScriptIgnoringException(const String& script, bool forceUserGesture)
#endif
{
#if PLATFORM(WKC)
    return executeScriptInWorldIgnoringException(mainThreadNormalWorld(), script, forceUserGesture, thisObject);
#else
...
#if PLATFORM(WKC)
JSC::JSValue ScriptController::executeScriptInWorldIgnoringException(DOMWrapperWorld& world, const String& script, bool forceUserGesture, Node* thisObject)
#else
...
{
#if PLATFORM(WKC)
    auto result = executeScriptInWorld(world, { script, URL { }, false, std::nullopt, forceUserGesture, RemoveTransientActivation::No }, thisObject);
#else
...
#if PLATFORM(WKC)
ValueOrException ScriptController::executeScriptInWorld(DOMWrapperWorld& world, RunJavaScriptParameters&& parameters, Node* thisObject)
#else
...
#if PLATFORM(WKC)
        return evaluateInWorld({ parameters.source, WTFMove(sourceURL), TextPosition(), JSC::SourceProviderSourceType::Program, CachedScriptFetcher::create(m_frame.document()->charset()) }, world, thisObject);
#else
...
#if PLATFORM(WKC)
        return callInWorld(WTFMove(parameters), world, thisObject);
#else
...
#if PLATFORM(WKC)
ValueOrException ScriptController::callInWorld(RunJavaScriptParameters&& parameters, DOMWrapperWorld& world, Node* thisObject)
#else
...
#endif
{
...
#if PLATFORM(WKC)
    JSValue thiValue = &proxy;
    if (thisObject) {
        thiValue = jsValueForNode(world, *thisObject);
        if (!thiValue)
            return makeUnexpected(ExceptionDetails { "Cannot convert JSValue for Node"_s });
    }
#endif
    ...
    do {
#if PLATFORM(WKC)
        JSValue functionObject = JSExecState::profiledEvaluate(&globalObject, JSC::ProfilingReason::Other, jsSourceCode, thiValue, evaluationException);
#else
...
#if PLATFORM(WKC)
JSC::JSValue ScriptController::executeUserAgentScriptInWorldIgnoringException(DOMWrapperWorld& world, const String& script, bool forceUserGesture, Node* thisObject)
#else
...
{
#if PLATFORM(WKC)
    auto result = executeUserAgentScriptInWorld(world, script, forceUserGesture, thisObject);
#else
...
#if PLATFORM(WKC)
ValueOrException ScriptController::executeUserAgentScriptInWorld(DOMWrapperWorld& world, const String& script, bool forceUserGesture, Node* thisObject)
#else
...
{
#if PLATFORM(WKC)
    return executeScriptInWorld(world, { script, URL { }, false, std::nullopt, forceUserGesture, RemoveTransientActivation::No }, thisObject);
#else
...
#if PLATFORM(WKC)
ValueOrException ScriptController::executeUserAgentScriptInWorldInternal(DOMWrapperWorld& world, RunJavaScriptParameters&& parameters, Node* thisObject)
#else
...
#if PLATFORM(WKC)
    return executeScriptInWorld(world, WTFMove(parameters), thisObject);
#else
...
#if PLATFORM(WKC)
void ScriptController::executeAsynchronousUserAgentScriptInWorld(DOMWrapperWorld& world, RunJavaScriptParameters&& parameters, ResolveFunction&& resolveCompletionHandler, Node* thisObject)
#else
...
#if PLATFORM(WKC)
    auto result = executeUserAgentScriptInWorldInternal(world, WTFMove(parameters), thisObject);
#else
