...
#if PLATFORM(WKC)
        if (roleValue.isNull())
            roleValue.construct("navigation", AtomString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC)
        if (idValue.isNull())
            idValue.construct("guide-inner-content", AtomString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC)
        if (class1.isNull())
            class1.construct("tree-select", AtomString::ConstructFromLiteral);
#endif
        static MainThreadNeverDestroyed<const AtomString> class2("v-tree-select", AtomString::ConstructFromLiteral);
#if PLATFORM(WKC)
        if (class2.isNull())
            class2.construct("v-tree-select", AtomString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC)
            if (instreamNativeVideoDivClass.isNull())
                instreamNativeVideoDivClass.construct("instream-native-video--mobile", AtomString::ConstructFromLiteral);
#endif
            static MainThreadNeverDestroyed<const AtomString> videoElementID("vjs_video_3_html5_api", AtomString::ConstructFromLiteral);
#if PLATFORM(WKC)
            if (videoElementID.isNull())
                videoElementID.construct("vjs_video_3_html5_api", AtomString::ConstructFromLiteral);
#endif
