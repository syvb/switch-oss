...
#else
        return (element() && element()->isLink() && forVisitedLink == ForVisitedLink::Yes) ? document().visitedLinkColor(m_style) : document().linkColor(m_style);
#endif
    case CSSValueWebkitActivelink:
#if !PLATFORM(WKC)
        return document().activeLinkColor();
#else
        return document().activeLinkColor(m_style);
#endif
