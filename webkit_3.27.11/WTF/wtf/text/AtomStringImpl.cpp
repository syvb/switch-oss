...
#if PLATFORM(WKC)
    if (string.isAtom()) {
        WTFLogAlways("Error: AtomStringImpl should not hit the slow case if the string is already an atom.");
        CRASH();
    }
#endif
...
void AtomStringImpl::remove(AtomStringImpl* string)
{
    ...
    if (iterator == atomStringTable.end()) {
        WTFLogAlways("Error: The string being removed is an atom in the string table of an other thread!");
        CRASH();
    }
#endif
...
#if PLATFORM(WKC)
    if (string.isAtom()) {
        WTFLogAlways("Error: AtomStringImpl objects should return from the fast case.");
        CRASH();
    }
#endif
