...
#include "DisplayRefreshMonitorFactoryWKC.h"
...
#include "helpers/WKCFocusOptions.h"
...
WebCore::Page*
ChromeClientWKC::createWindow(WebCore::Frame& frame, const WebCore::WindowFeatures& features, const WebCore::NavigationAction& navact)
{
    FramePrivate fp(&frame);
    WKC::Page* ret = m_appClient->createWindow(&fp.wkc(), (const WKC::WindowFeatures &)features);
    if (!ret)
        return 0;
...
void
ChromeClientWKC::print(WebCore::Frame& frame, const WebCore::StringWithDirection&)
{
...
void
ChromeClientWKC::elementDidFocus(WebCore::Element& element, const WebCore::FocusOptions& options)
{
    WKC::FocusOptions op;
    op.selectionRestorationMode = toWKCSelectionRestorationMode(options.selectionRestorationMode);
    op.direction = toWKCFocusDirection(options.direction);
    op.preventScroll = options.preventScroll;

    ElementPrivate* e = ElementPrivate::create(&element);
    m_appClient->elementDidFocus(&e->wkc(), op);
    delete e;
}
...
void
ChromeClientWKC::elementDidRefocus(WebCore::Element& element, const WebCore::FocusOptions& options)
{
    WKC::FocusOptions op;
    op.selectionRestorationMode = toWKCSelectionRestorationMode(options.selectionRestorationMode);
    op.direction = toWKCFocusDirection(options.direction);
    ...
    ElementPrivate* e = ElementPrivate::create(&element);
    m_appClient->elementDidRefocus(&e->wkc(), op);
    delete e;
}
...
{
    auto p = handle.platformCursor();
    m_appClient->setCursor(p.get());
}
...
void
ChromeClientWKC::scrollContainingScrollViewsToRevealRect(const WebCore::IntRect& rect) const
{
    m_appClient->scrollContainingScrollViewsToRevealRect(rect);
}
...
{
    return WebCore::Color(WebCore::asSRGBA(WebCore::PackedColor::ARGB{ m_appClient->underlayColor() }));
}
...
void
ChromeClientWKC::dispatchApplyOverflowToViewport(WebCore::Frame& frame, bool overflowXHidden, bool overflowYHidden) const
{
    FramePrivate fp(&frame);
    m_appClient->dispatchApplyOverflowToViewport(&fp.wkc(), overflowXHidden, overflowYHidden);
}

bool
ChromeClientWKC::dispatchWillSetHasFocusVisibleForFocusedElement(WebCore::Element& focusedElement, bool value) const
{
    WKC::ElementPrivate fe(&focusedElement);
    return m_appClient->dispatchWillSetHasFocusVisibleForFocusedElement(fe.wkc(), value);
}
...
void
ChromeClientWKC::exitVideoFullscreenForVideoElement(WebCore::HTMLVideoElement& element, WTF::CompletionHandler<void(bool)>&& completionHandler)
{
...
WebCore::DisplayRefreshMonitorFactory*
ChromeClientWKC::displayRefreshMonitorFactory() const
{
    static LazyNeverDestroyed<WebCore::DisplayRefreshMonitorFactoryWKC> factory;
    if (factory.isNull())
        factory.construct();
    return &factory.get();
}
...
void
ChromeClientWKC::triggerRenderingUpdate()
{
    m_appClient->triggerRenderingUpdate();
}
...
bool ChromeClientWKC::hoverSupportedByPrimaryPointingDevice() const
{
    notImplemented();
    return false;
}

bool ChromeClientWKC::hoverSupportedByAnyAvailablePointingDevice() const
{
    notImplemented();
    return false;
}

std::optional<WebCore::PointerCharacteristics> ChromeClientWKC::pointerCharacteristicsOfPrimaryPointingDevice() const
{
    notImplemented();
    return std::nullopt;
}

OptionSet<WebCore::PointerCharacteristics> ChromeClientWKC::pointerCharacteristicsOfAllAvailablePointingDevices() const
{
    notImplemented();
    return WebCore::PointerCharacteristics::Coarse;
}

void ChromeClientWKC::setTextIndicator(const WebCore::TextIndicatorData&) const
{
    notImplemented();
}

void ChromeClientWKC::requestCookieConsent(CompletionHandler<void(WebCore::CookieConsentDecisionResult)>&&)
{
    notImplemented();
}

void ChromeClientWKC::classifyModalContainerControls(Vector<WTF::String>&& texts, CompletionHandler<void(Vector<WebCore::ModalContainerControlType>&&)>&&)
{
    notImplemented();
}

void ChromeClientWKC::decidePolicyForModalContainer(OptionSet<WebCore::ModalContainerControlType>, CompletionHandler<void(WebCore::ModalContainerDecision)>&&)
{
    notImplemented();
}
