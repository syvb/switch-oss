...
#include "StorageSessionProviderWKC.h"
...
extern "C" {
void tc_malloc_resetVariables(void);
void curl_global_reset_variables(void);
void icu4c_resetVariables(void);
void libxml2_resetVariables(void);
void libxslt_resetVariables(void);
void sqlite3_force_terminate(void);
void pixman_resetVariables(void);
#if USE(AVIF)
void dav1d_resetVariables(void);
#endif
cairo_public void cairo_resetVariables(void);
...
static bool gIsForceTerminatingAllowed = false;
       ...
       m_clientBuilders(builders)
{
    ...
    m_desktopSize = WebCore::IntSize();
    m_viewSize = WebCore::IntSize();
    m_defaultDesktopSize = WebCore::IntSize();
    m_defaultViewSize = WebCore::IntSize();
...
WKCWebViewPrivate::~WKCWebViewPrivate()
{
    ...
    if (m_corePage) {
        m_corePage->settings().setUsesBackForwardCache(false);
...
WKCWebViewPrivate::construct()
{
    ...
    WebCore::PageConfiguration cli(WebCore::pageConfigurationWithEmptyClients(PAL::SessionID::defaultSessionID()));
    ...
    cli.dragClient.reset(drag);
    cli.inspectorClient = m_inspector;
    cli.progressTrackerClient = WTF::makeUniqueRef<ProgressTrackerClientWKC>(WTFMove(*m_progress));
    m_progress = nullptr;
    cli.loaderClientForMainFrame = UniqueRef<WebCore::FrameLoaderClient>(*mainframeloaderclient);
    cli.alternativeTextClient = nullptr;
    cli.databaseProvider = &WebDatabaseProvider::singleton();
    cli.cookieJar = WebCore::CookieJar::create(WKC::StorageSessionProviderWKC::create());
    ...
    m_corePage->addLayoutMilestones(OptionSet<WebCore::LayoutMilestone>::fromRaw(WebCore::DidFirstLayout | WebCore::DidFirstVisuallyNonEmptyLayout));
...
//    settings->setAllowScriptsToCloseWindows(true);
    ...
    settings->setFixedBackgroundsPaintRelativeToDocument(false);
    ...
    settings->setCSSOMViewScrollingAPIEnabled(true);
     ...
     settings->setMaximumSourceBufferSize(INT_MAX);
#endif
...
#endif
#if ENABLE(GAMEPAD)
     settings->setGamepadsEnabled(true);
#endif
    ...
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setPaintTimingEnabled(true);
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setServerTimingEnabled(true);
...
WKCWebViewPrivate::notifyResizeDesktopSize(const WebCore::IntSize& size, bool sendresizeevent)
{
    ...
    if (sendresizeevent)
        frame->view()->scheduleResizeEventIfNeeded();
    frame->view()->forceLayout();
    frame->view()->adjustViewSize();
...
void
WKCWebViewPrivate::setSizeForCSSDefaultViewportUnits(const WebCore::FloatSize& size)
{
    WebCore::Frame* frame = (WebCore::Frame *)&core()->mainFrame();
    ...
    frame->view()->setSizeForCSSDefaultViewportUnits(size);
}

void
WKCWebViewPrivate::setSizeForCSSSmallViewportUnits(const WebCore::FloatSize& size)
{
    WebCore::Frame* frame = (WebCore::Frame *)&core()->mainFrame();
    ...
    frame->view()->setSizeForCSSSmallViewportUnits(size);
}

void
WKCWebViewPrivate::setSizeForCSSLargeViewportUnits(const WebCore::FloatSize& size)
{
    WebCore::Frame* frame = (WebCore::Frame *)&core()->mainFrame();
    ...
    frame->view()->setSizeForCSSLargeViewportUnits(size);
}

void
WKCWebViewPrivate::setLayoutViewportOverrideRect(const WebCore::LayoutRect& rect)
{
    WebCore::Frame* frame = (WebCore::Frame *)&core()->mainFrame();
    ...
    frame->view()->setLayoutViewportOverrideRect(rect);
}
...
void
WKCWebViewPrivate::notifyPaintOffscreenFrom(const WebCore::IntRect& rect, const WKCFloatPoint& p)
{
#if USE(WKC_CAIRO)
    ...
    PlatformGraphicsContext *ctx = static_cast<PlatformGraphicsContext*>(m_drawContext);
...
WKCWebViewPrivate::notifyPaintToContext(const WebCore::IntRect& rect, void* context)
{
    ...
    PlatformGraphicsContext ctx(static_cast<cairo_t*>(context));
...
{
    WebCore::Element* element = core()->focusController().getFocusedElement();
    if (!element)
        return nullptr;
...
WKCWebViewPrivate::getElementFromPoint(int x, int y)
{
    ...
    while (frame) {
        ...
        WebCore::HitTestRequest request({ WebCore::HitTestRequest::Type::ReadOnly , WebCore::HitTestRequest::Type::Active });
        ...
        frame = WebCore::EventHandler::subframeForHitTestResult(hr).get();
    }
...
WKCWebViewPrivate::selectionText()
{
    ...
    m_selectedText = WebCore::plainText(*range);

    return reinterpret_cast<const unsigned short*>(m_selectedText.charactersWithNullTermination().data());
}
...
{
    if (isVisible) {
        m_activityState.remove(WebCore::ActivityState::IsVisuallyIdle);
        m_activityState.add(WebCore::ActivityState::IsVisible);
        m_activityState.add(WebCore::ActivityState::IsVisibleOrOccluded);
        ...
        m_activityState.remove(WebCore::ActivityState::IsVisible);
        m_activityState.remove(WebCore::ActivityState::IsVisibleOrOccluded);
        m_activityState.add(WebCore::ActivityState::IsVisuallyIdle);
        // Stop SVG animations.
        for (WebCore::Frame* frame = &core()->mainFrame(); frame; frame = frame->tree().traverseNext()) {
            if (frame->document() && frame->document()->svgExtensions())
...
void
WKCWebViewPrivate::setUseDarkAppearance(bool useDarkAppearance)
{
#if ENABLE(DARK_MODE_CSS)
    core()->effectiveAppearanceDidChange(useDarkAppearance, false);
#else
...
bool
WKCWebViewPrivate::useDarkAppearance() const
{
#if ENABLE(DARK_MODE_CSS)
    return core()->useDarkAppearance();
#else
    return false;
#endif
}
...
{
    m_activityState.add(WebCore::ActivityState::WindowIsActive);
    m_activityState.add(WebCore::ActivityState::IsFocused);
    core()->setActivityState(m_activityState);
}
...
{
    m_activityState.remove(WebCore::ActivityState::WindowIsActive);
    m_activityState.remove(WebCore::ActivityState::IsFocused);
    core()->setActivityState(m_activityState);
}

bool
WKCWebViewPrivate::dispatchNavigationEvent(WKC::NavigationEventType type, WKC::Element* target, WKC::Element* relatedTarget, WKC::FocusDirection dir, bool* defaultPrevented)
{
    WebCore::Element* coreTarget = target ? static_cast<WebCore::Element*>(target->priv().webcore()) : nullptr;
    WebCore::Element* coreRelatedTarget = relatedTarget ? static_cast<WebCore::Element*>(relatedTarget->priv().webcore()) : nullptr;
    WebCore::Element* focusedElement = core()->focusController().getFocusedElement();
    WebCore::Document* document = coreTarget ? &coreTarget->document() : core()->mainFrame().document();
    WebCore::SpatialNavigationDirection coreDir;
    const WTF::AtomString* eventName = nullptr;

    // https://www.w3.org/TR/css-nav-1/#events-nav-type
    if (!coreTarget) {
        coreTarget = document->body() ? document->body() : document->documentElement();
        if (!coreTarget)
            return false;
    }

    if (!convertToSpatialNavigationDirection(dir, coreDir)) {
        ASSERT_NOT_REACHED();
        return false;
    }

    switch (type) {
    case NavigationEventType::navbeforefocusEvent:
        // ref. WebCore::Document::setFocusedElement()
        {
            if (!coreRelatedTarget || coreRelatedTarget == focusedElement)
                return false;

            if (document->backForwardCacheState() != WebCore::Document::NotInBackForwardCache)
                return false;
        }
        eventName = &WebCore::eventNames().navbeforefocusEvent;
        break;
    case NavigationEventType::navnotargetEvent:
        eventName = &WebCore::eventNames().navnotargetEvent;
        break;
    default:
        ASSERT_NOT_REACHED();
        return false;
    ...
    auto event = WebCore::NavigationEvent::create(*eventName,
                                                  WebCore::Event::CanBubble::Yes,
                                                  WebCore::Event::IsCancelable::Yes,
                                                  document->windowProxy(),
                                                  0,
                                                  coreDir,
                                                  coreRelatedTarget);
    coreTarget->dispatchScopedEvent(event);
...
void
WKCWebView::setSizeForCSSDefaultViewportUnits(const WKCFloatSize& size)
{
    WebCore::FloatSize s(size.fWidth, size.fHeight);
    m_private->setSizeForCSSDefaultViewportUnits(s);
}

void
WKCWebView::setSizeForCSSSmallViewportUnits(const WKCFloatSize& size)
{
    WebCore::FloatSize s(size.fWidth, size.fHeight);
    m_private->setSizeForCSSSmallViewportUnits(s);
}

void
WKCWebView::setSizeForCSSLargeViewportUnits(const WKCFloatSize& size)
{
    WebCore::FloatSize s(size.fWidth, size.fHeight);
    m_private->setSizeForCSSLargeViewportUnits(s);
}

void
WKCWebView::setLayoutViewportOverrideRect(const WKCFloatRect& rect)
{
    WebCore::LayoutRect r(rect.fX, rect.fY, rect.fWidth, rect.fHeight);
    m_private->setLayoutViewportOverrideRect(r);
}
...
void
WKCWebView::notifyPaintOffscreenFrom(const WKCRect& rect, const WKCFloatPoint& p)
{
...
bool
WKCWebView::notifyKeyEvent(WKC::KeyEventType in_type, WKC::Key in_key, unsigned int in_char, WKC::Modifier in_modifiers, bool in_autorepeat)
{
    ...
    if (in_confirm) {
        ...
        if (in_underlineNum > 0) {
            ...
            for (unsigned i = 0; i < in_underlineNum; i++) {
                ...
                underlines[i].color = WebCore::asSRGBA(WebCore::PackedColor::ARGB{ in_underlines[i].color });
...
WKCWebView::notifyMouseMoveTest(const WKCPoint& pos, WKC::MouseButton button, Modifier modifiers, bool& contentChanged)
{
    ...
    if (renderer && WTF::is<WebCore::RenderElement>(renderer)) {
        renderer = ((WebCore::RenderElement *)renderer)->parent();
    } else {
        renderer = 0;
    }
...
bool
WKCWebView::notifyMouseWheel(const WKCPoint& pos, const WKCFloatSize& diff, WKC::Modifier modifiers)
{
    WebCore::Frame* frame = (WebCore::Frame *)&m_private->core()->mainFrame();
    ...
    WTF::OptionSet<WebCore::WheelEventProcessingSteps> option;
    option.add(WebCore::WheelEventProcessingSteps::MainThreadForBlockingDOMEventDispatch); // cancelable

    return frame->eventHandler().handleWheelEvent(wheelEvent, option);
}
...
void
WKCWebView::notifyPointerEvent(int type, const TouchPoint* points, int npoints, WKC::Modifier in_modifiers)
{
#if ENABLE(TOUCH_EVENTS)
    WebCore::Page* page = m_private->core();

    if (type == WKC::EPointerTypeReset) {
        page->pointerCaptureController().reset();
        return;
    }

    for (int i=0; i<npoints; i++) {
        if (type == WKC::EPointerTypeCancel) {
            page->pointerCaptureController().cancelPointer(points[i].fId, WebCore::IntPoint(points[i].fPoint.fX, points[i].fPoint.fY));
            continue;
        }

        WKC::Element* element = getElementFromPoint(points[i].fPoint.fX, points[i].fPoint.fY);
        if (!element)
            continue;
        WebCore::Node* target = element->priv().webcore();
        if (!target)
            continue;
        WebCore::WindowProxy* view = target->document().windowProxy();
        if (!view)
            continue;

        WKC::WKCTouchEvent ev = {0};
        WTF::Vector<WKCTouchPoint> tp(1);

        tp[0].m_id = points[i].fId;
        tp[0].m_state = points[i].fState;
        WKCPoint_SetPoint(&tp[0].m_pos, &points[i].fPoint);
        ...
        page->pointerCaptureController().dispatchEventForTouchAtIndex(*target, touchEvent, i, true, *view);
    }
#endif
}
...
WKCWebView::notifyScroll(WKC::ScrollType type)
{
    ...
    WebCore::ScrollGranularity gra = WebCore::ScrollGranularity::Line;

    switch (type) {
    case EScrollUp:
        gra = WebCore::ScrollGranularity::Line;
    ...
    case EScrollRight:
        ...
        break;
    case EScrollPageUp:
        gra = WebCore::ScrollGranularity::Page;
        dir = WebCore::ScrollUp;
        break;
    ...
    case EScrollPageRight:
        ...
        break;
    case EScrollTop:
        gra = WebCore::ScrollGranularity::Document;
        dir = WebCore::ScrollUp;
        break;
    case EScrollBottom:
        gra = WebCore::ScrollGranularity::Document;
...
WKCWebView::setFocusedElement(WKC::Element* ielement)
{
    ...
    if (element && frame) {
        ...
        if (focusedDocument->parentDocument()) {
            focusedDocument->parentDocument()->setFocusedElement(0);
        }
        if (newDocument)
...
WKCWebView::searchText(const unsigned short* text, bool case_sensitive, bool forward, bool wrap)
{
    ...
    if (!case_sensitive)
        opts.add(WebCore::CaseInsensitive);
    if (!forward)
        opts.add(WebCore::Backwards);
    if (wrap)
        opts.add(WebCore::WrapAround);
...
void
WKCWebView::executeScriptIgnoringException(const char* script, bool forceUserGesture, Node* thisObject)
{
    m_private->core()->mainFrame().script().executeScriptIgnoringException(WTF::String::fromUTF8(script), forceUserGesture, thisObject ? thisObject->priv().webcore() : nullptr);
}

void
WKCWebView::executeUserAgentScriptInWorldIgnoringException(DOMWrapperWorld& world, const char* script, bool forceUserGesture, Node* thisObject)
{
    if (world.priv().webcore())
        m_private->core()->mainFrame().script().executeUserAgentScriptInWorldIgnoringException(*world.priv().webcore(), WTF::String::fromUTF8(script), forceUserGesture, thisObject ? thisObject->priv().webcore() : nullptr);
}
...
bool
WKCWebView::isCaretAtEndingOfTextField() const
{
    WebCore::Frame* frame = (WebCore::Frame *)&m_private->core()->focusController().focusedOrMainFrame();
    if (!frame || !frame->document())
        return false;
    ...
    const WebCore::VisiblePosition& endPosition = frameSelection.selection().visibleEnd();

    // cf. WebCore::isLastVisiblePositionInNode

    if (endPosition.isNull())
        return false;

    if (!endPosition.deepEquivalent().containerNode()->isDescendantOrShadowDescendantOf(element))
        return false;

    WebCore::VisiblePosition next = endPosition.next();
    return next.isNull() || !next.deepEquivalent().deprecatedNode()->isDescendantOrShadowDescendantOf(element);
}
...
_selectionRects(WebCore::Page* page, WTF::Vector<WebCore::IntRect>& rects, bool textonly, bool useSelectionHeight)
{
    ...
    WebCore::IntersectingNodeRange intersecting = intersectingNodes(*range);
    auto stopNode = intersecting.end();
    for (auto node = intersecting.begin(); node && node != stopNode; ++node) {
...
WKCWebView::encoding()
{
    ...
    m_private->m_encoding = wkc_wstrdup(reinterpret_cast<const unsigned short*>(encoding.charactersWithNullTermination().data()));
...
WKCWebView::customEncoding()
{
    ...
    m_private->m_customEncoding = wkc_wstrdup(reinterpret_cast<const unsigned short*>(overrideEncoding.charactersWithNullTermination().data()));
...
WKCWebView::releaseMemory(ReleaseMemoryType type, bool is_synchronous)
{
    ...
    if (type & EMemoryTypeNoncriticalFontWidthCache) {
        WebCore::FontCache::forCurrentThread().clearWidthCaches();
    }
    ...
    if (type & EMemoryTypeCriticalPageCache) {
        WebCore::BackForwardCache::singleton().pruneToSizeNow(0, WebCore::PruningReason::None);
    }
...
WKCWebView::isScrollbarFromPoint(int x, int y, ScrollbarPart& part, WKCRect& ir)
{
    ...
    while (frame) {
        ...
        frame = WebCore::EventHandler::subframeForTargetNode(node).get();
    }
...
bool WKCWebView::addVisitedLink(const char* uri, const unsigned short* title, const struct tm* date)
{
    ...
    WebCore::SharedStringHash hash = WebCore::computeSharedStringHash(url.string());
...
{
    URL ki(URL(), WTF::String::fromUTF8(uri));
    WTF::String domain = ki.host().toString();
    WTF::String path = ki.path().toString();
...
void
WKCWebKitSetMainThreadLongjmpProc(MainThreadLongjmpProc in_proc)
{
    wkcThreadSetMainThreadLongjmpProcPeer(in_proc);
}
...
public:
    ...
    bool checkMemoryAllocatable(size_t request_size, WKCMemoryEventHandler::AllocationReason reason) {
        return m_memoryEventHandler.checkMemoryAllocatable(request_size, reason);
    }
    void notifyMemoryExhaust(size_t request_size, WKCMemoryEventHandler::HeapType heap_type) {
        m_memoryEventHandler.notifyMemoryExhaust(request_size, heap_type);
    }
    void notifyMemoryAllocationError(size_t request_size, WKCMemoryEventHandler::AllocationReason reason) {
    ...
    void notifyStackOverflow(size_t stack_size, size_t consumption, size_t margin, void* stack_top, void* stack_base, void* current_stack_top, const char* file, int line, const char* function) {
        m_memoryEventHandler.notifyStackOverflow(stack_size, consumption, margin, stack_top, stack_base, current_stack_top, file, line, function);
    }
    bool allocateMemoryPages(void* ptr, size_t size) {
        return m_memoryEventHandler.allocateMemoryPages(ptr, size);
    }
    void freeMemoryPages(void* ptr, size_t size) {
        m_memoryEventHandler.freeMemoryPages(ptr, size);
    }
...
static void
WKCWebKitNotifyNoMemory(size_t request_size)
{
    gMemoryEventHandler->notifyMemoryExhaust(request_size, WKCMemoryEventHandler::HeapType::Main);
}
...
WKCWebKitNotifyFontNoMemory()
{
    ...
    gMemoryEventHandler->notifyMemoryExhaust(0, WKCMemoryEventHandler::HeapType::Font);

    if (!wkcThreadCheckAlivePeer()) { // Basically, the current thread must be terminated here.
        // Never reach here.
    }
}
...
WKCWebKitConvertAllocationReason(int in_reason)
{
    ...
    case WKC_MEMORYALLOC_TYPE_IMAGE:
        result = WKCMemoryEventHandler::AllocationReason::Image;
        break;
    case WKC_MEMORYALLOC_TYPE_JAVASCRIPT:
        result = WKCMemoryEventHandler::AllocationReason::JavaScript;
        break;
#if USE(WKC_CAIRO)
    case WKC_MEMORYALLOC_TYPE_PIXMAN:
        result = WKCMemoryEventHandler::AllocationReason::Pixman;
        break;
#endif
    default:
        ASSERT_NOT_REACHED();
...
static void
WKCWebKitNotifyMemoryAllocationError(size_t request_size, int in_reason)
{
...
static void
WKCWebKitNotifyStackOverflow(size_t stack_size, size_t consumption, size_t margin, void* stack_top, void* stack_base, void* current_stack_top, const char* file, int line, const char* function)
{
    gMemoryEventHandler->notifyStackOverflow(stack_size, consumption, margin, stack_top, stack_base, current_stack_top, file, line, function);
}

static bool
WKCWebKitCheckMemoryAllocatable(size_t request_size, int in_reason)
{
    return gMemoryEventHandler->checkMemoryAllocatable(request_size, WKCWebKitConvertAllocationReason(in_reason));
}

static bool
WKCWebKitAllocateMemoryPages(void* ptr, size_t size)
{
    return gMemoryEventHandler->allocateMemoryPages(ptr, size);
}

static void
WKCWebKitFreeMemoryPages(void* ptr, size_t size)
{
    gMemoryEventHandler->freeMemoryPages(ptr, size);
}
...
static void*
peer_malloc_aligned_proc(unsigned int in_size, unsigned int in_align, int in_crashonfailure)
{
    void* ptr = 0;
    if (in_crashonfailure) {
        ptr = WTF::fastAlignedMalloc(in_align, in_size);
    } else {
        ptr = WTF::tryFastAlignedMalloc(in_align, in_size);
    }
    return ptr;
}
...
bool
shouldInterruptJavaScript(JSC::JSGlobalObject*, void*, void*)
{
...
bool
WKCWebKitInitialize(void* memory, size_t physical_memory_size, size_t virtual_memory_size, void* font_memory, size_t font_memory_size, WKCMemoryEventHandler& memory_event_handler)
{
    ...
    wkcMemorySetAllocateMemoryPagesProcPeer(WKCWebKitAllocateMemoryPages);
    wkcMemorySetFreeMemoryPagesProcPeer(WKCWebKitFreeMemoryPages);
...
#endif
    JSC::Options::forceRAMSize() = wkcMemoryGetMaxPhysicalMemorySizePeer();
    ...
    PAL::atomCanonicalTextEncodingName("UTF-8");
    ...
    WebCore::BackForwardCache::singleton().setMaxSize(0); /* dont use page cache now*/
    ...
    gIsForceTerminatingAllowed = true;
...
WKCWebKitFinalize()
{
    gIsForceTerminatingAllowed = false;

    wkcThreadSetForceTerminateFlagPeer(WKC_THREAD_FORCE_TERMINATE_FLAG_FINALIZING);
...
bool WKCWebKitGetFontFamilyName(int id, char* name, int len)
{
    return wkcFontEngineGetFamilyNamePeer(id, name, len);
}
...
WKC_API void
WKCWebKitTriggerForceTerminate()
{
    if (!gIsForceTerminatingAllowed) {
        return;
    }

    wkcThreadSetForceTerminateFlagPeer(WKC_THREAD_FORCE_TERMINATE_FLAG_TRIGGERED);

    {
        // Allow longjmp even if the engine main thread is in a busy loop.
        JSC::VM& vm = WebCore::commonVM();
        // Do not invoke lock here.
        vm.notifyTriggerForceTerminate();
    }

    if (!wkcThreadCurrentThreadPeer()) {
        // If the current thread is neither the engine main thread nor an engine sub-thread, do nothing.
        return;
    }

    if (!wkcThreadCheckAlivePeer()) { // The current thread is terminated here.
        // Should not reach here.
        return;
    }
}
...
WKCResetVariables()
{
    tc_malloc_resetVariables();
    curl_global_reset_variables();
    icu4c_resetVariables();
    libxml2_resetVariables();
#if ENABLE(XSLT)
    libxslt_resetVariables();
#endif
    pixman_resetVariables();
    cairo_resetVariables();
#if ENABLE(GRAPHICS_CONTEXT_3D)
    ShResetVariables();
#endif
#if USE(AVIF)
    dav1d_resetVariables();
#endif
...
void
WKCWebKitRequestGarbageCollect(bool is_now, bool is_full)
{
    if (is_now) {
        WebCore::GCController::singleton().garbageCollectNow(is_full);
        WebCore::GCController::singleton().releaseFreeRegionsInHeap();
    } else {
        WebCore::GCController::singleton().garbageCollectSoon(); // always full GC
    }
}
...
WKCWebKitStartWebInspector(const char* addr, int port, bool(*modalcycle)(void*), void* opaque)
{
    ...
    if (success) {
        WTF::RunLoop::setCycleProc(modalcycle, opaque);
...
WKCWebKitStopWebInspector()
{
    if (WebInspectorServer::sharedInstance()) {
        WebInspectorServer::deleteSharedInstance();
        WTF::RunLoop::setCycleProc(nullptr, nullptr);
    }
}
...
WKCWebView::scrollNodeByRecursively(WKC::Node* node, int dx, int dy)
{
    ...
    coreNode->renderer()->enclosingLayer()->scrollableArea()->scrollByRecursively(WebCore::IntSize(dx, dy));
}
...
WKCWebView::scrollNodeBy(WKC::Node* node, int dx, int dy)
{
    ...
    if (renderer->hasNonVisibleOverflow() && isLineClampNoneByParent) {
        WebCore::RenderLayerScrollableArea* scrollable = layer->ensureLayerScrollableArea();
        WebCore::ScrollOffset offset = scrollable->scrollOffset();
        int offsetX = offset.x() + dx;
        int offsetY = offset.y() + dy;
        scrollable->scrollToOffset(WebCore::ScrollOffset(offsetX, offsetY));
...
WKCWebView::getScrollableContentRect(const WKC::Node* scrollableNode)
{
    ...
    if (WTF::is<WebCore::HTMLFrameOwnerElement>(node)) {
        ...
        if (!frameOwnerElement->renderer() || !frameOwnerElement->contentFrame() || frameOwnerElement->scrollingMode() == WebCore::ScrollbarMode::AlwaysOff) {
            return WKCFloatRect();
        }
    ...
    if (WTF::is<WebCore::Document>(node)) {
        WebCore::FloatRect rect = box->absoluteContentQuad().boundingBox();
        if (WebCore::HTMLFrameOwnerElement* owner = downcast<WebCore::Document>(node)->ownerElement()) {
            if (!owner->renderer() || !owner->renderer()->isBox())
                return WKCFloatRect();
    ...
    if (box->scrollsOverflow()) {
        WebCore::FloatRect rect = box->clientBoxRect();
...
WKCFloatRect
WKCWebView::transformedRect(const WKC::Node* node, const WKCFloatRect& rect)
{
    if (!node) {
        return rect;
    }
    ...
    WebCore::FloatRect r(rect.fX - absPos.x(), rect.fY - absPos.y(), rect.fWidth, rect.fHeight);
    WebCore::FloatQuad quad = renderer->localToAbsoluteQuad(WebCore::FloatQuad(r));
    WebCore::FloatRect br = quad.boundingBox();
...
WKCWebView::cancelFullScreen()
{
    ...
    for (WebCore::Frame* frame = &m_private->core()->mainFrame(); frame; frame = frame->tree().traverseNext()) {
        if (frame->document()->fullscreenManager().fullscreenElement())
            childmost = frame->document();
    }
    if (childmost)
        childmost->fullscreenManager().cancelFullscreen();
}
...
WKCWebView::localStorageMemoryConsumptionBytes(const char* pagegroupname)
{
    ...
    unsigned bytes = 0;
    const WebCore::Storage* storage = 0;
    const WTF::WeakHashSet<WebCore::Page>& pages = pageGroup->pages();
...
WKCWebViewPrivate::addUserContentExtension(const char* name, const char* extension)
{
    ...
    auto err = WebCore::ContentExtensions::compileRuleList(*contentExtensionClient, WTFMove(json));
    if (err)
        return false;
...
WKCWebViewPrivate::addCompiledUserContentExtension(const char* name, const unsigned char* filtersWithoutDomainsBytecode, unsigned int filtersWithoutDomainsBytecodeLen, const unsigned char* filtersWithDomainsBytecode, unsigned int filtersWithDomainsBytecodeLen, const unsigned char* domainFiltersBytecode, unsigned int domainFiltersBytecodeLen, const unsigned char* actions, unsigned int actionsLen)
{
    ...
    contentExtensionClient->writeFiltersWithoutDomainsBytecode(WTFMove(filtersWithoutDomains));
    ...
    contentExtensionClient->writeFiltersWithDomainsBytecode(WTFMove(filtersWithDomains));
    ...
    contentExtensionClient->writeDomainFiltersBytecode(WTFMove(domainFilter));

    WTF::Vector<WebCore::ContentExtensions::SerializedActionByte> vaction(actionsLen);
    vaction.append(actions, actionsLen);
    contentExtensionClient->writeActions(WTFMove(vaction));
...
void
WKCWebView::setUseDarkAppearance(bool useDarkAppearance)
{
    m_private->setUseDarkAppearance(useDarkAppearance);
}

bool
WKCWebView::useDarkAppearance() const
{
    return m_private->useDarkAppearance();
}

bool
WKCWebView::dispatchNavigationEvent(WKC::NavigationEventType type, WKC::Element* target, WKC::Element* relatedTarget, WKC::FocusDirection dir, bool* defaultPrevented)
{
    return m_private->dispatchNavigationEvent(type, target, relatedTarget, dir, defaultPrevented);
}
...
void
WKCWebViewPrivate::allowLayout()
{
    for (auto* document : WebCore::Document::allDocuments()) {
        document->disallowLayout(false);
    }
}

void
WKCWebViewPrivate::disallowLayout()
{
    for (auto* document : WebCore::Document::allDocuments()) {
        document->disallowLayout(true);
    }
}

void
WKCWebView::allowLayout()
{
    m_private->allowLayout();
}

void
WKCWebView::disallowLayout()
{
    m_private->disallowLayout();
}
...
WKCWebView::dumpExternalRepresentation(Frame* frame)
{
    ...
    WTF::String output = WebCore::externalRepresentation(frame->priv().webcore(), OptionSet<WebCore::RenderAsTextFlag>{WebCore::RenderAsTextFlag::ShowAllLayers
...
void
WKCWebView::getCurrentFocusPoint(float& x, float& y)
{
    WebCore::LayoutPoint p = m_private->core()->focusController().getCurrentFocusPoint();
    x = p.x();
    y = p.y();
}
...
{
    m_private->core()->focusController().clearCurrentFocusPoint();
}
...
void
WKCWebKitSetCurlDebugCallback(CurlDebugProc in_proc)
{
    WebCore::ResourceHandleManager::sharedInstance()->setCurlDebugCallback(in_proc);
}
...
{
    WebCore::DisplayRefreshMonitorManager::sharedManager().displayWasUpdated(0, { });
}

void
WKCWebKitInitializeLogChannelsIfNecessary(const char* logLevelString)
{
#if !LOG_DISABLED || !RELEASE_LOG_DISABLED
    if (logLevelString)
        WebCore::logChannels().initializeLogChannelsIfNecessary(WTF::String(logLevelString));
    else
        WebCore::logChannels().initializeLogChannelsIfNecessary(WTF::String("-all"));
#endif
}

bool
WKCWebKitSetICUData(const void* data)
{
...
{
    return wkcI18NIDNtoUnicodePeer((const unsigned char *)host, -1, (UChar *)idn, maxidn);
}

int
fromUnicode(const unsigned short* idn, char* host, int maxhost)
{
    return wkcI18NIDNfromUnicodePeer((UChar *)idn, -1, (unsigned char *)host, maxhost);
}
...
{
    auto encoded = WTF::base64EncodeToVector(Span<const std::byte>{ reinterpret_cast<const std::byte*>(in), ::strlen(in) });

    if (buf && buflen >= encoded.size())
        strncpy(buf, reinterpret_cast<char*>(encoded.data()), encoded.size());
