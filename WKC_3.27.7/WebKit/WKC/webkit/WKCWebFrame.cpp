...
WKCWebFramePrivate::~WKCWebFramePrivate()
{
    ...
    if (m_coreFrame) {
        m_coreFrame->navigationScheduler().cancel();
        if (!m_coreFrame->loader().isComplete())
            m_coreFrame->loader().closeURL();

        m_coreFrame->loader().clear(m_coreFrame->document(), false);
        m_coreFrame->script().updatePlatformScriptObjects();
...
WKCWebFramePrivate::construct()
{
    ...
    if (m_isMainFrame) {
        ...
        RefPtr<WebCore::Frame> newframe = WebCore::Frame::create(m_view->core(), parent, UniqueRef<WKC::FrameLoaderClientWKC>(*client));
        if (!newframe) {
            delete client;
            return false;
        }
...
WKCWebFrame::name()
{
    ...
    m_private->m_name = wkc_wstrdup(reinterpret_cast<const unsigned short*>(string.characters16()));
    return m_private->m_name;
}
const unsigned short*
WKCWebFrame::title()
...
WKCWebFrame::loadURI(const char* uri, const char* referrer)
{
    ...
    if (referrer) {
        ...
        WebCore::FrameLoadRequest req(*coreFrame, WebCore::ResourceRequest(URL(URL(), utf8Uri), refStr));
        ...
        WebCore::FrameLoadRequest req(*coreFrame, rr);
        coreFrame->loader().load(WTFMove(req));
    }
    return true;
}
...
WKCWebFrame::loadString(const char* content, const unsigned short* mime_type, const unsigned short* encoding, const char *base_uri, const char *unreachable_uri, bool replace)
{
    ...
    URL baseKURL = (base_uri && base_uri[0]) ? URL(URL(), WTF::String::fromUTF8(base_uri)) : WTF::aboutBlankURL();
    ...
    WebCore::FrameLoadRequest req(*coreFrame, request, substituteData);
...
WKCWebFrame::loadRequest(const WKC::ResourceRequest& request)
{
    ...
    WebCore::FrameLoadRequest req(*coreFrame, request.priv().webcore());
    coreFrame->loader().load(WTFMove(req));
}
...
void
WKCWebFrame::executeScriptIgnoringException(const char* script, bool forceUserGesture, Node* thisObject)
{
    WebCore::Frame* coreFrame = m_private->core();
    if (!coreFrame) {
        return;
    ...
    coreFrame->script().executeScriptIgnoringException(WTF::String::fromUTF8(script), forceUserGesture, thisObject ? thisObject->priv().webcore() : nullptr);
}

void
WKCWebFrame::executeUserAgentScriptInWorldIgnoringException(DOMWrapperWorld& world, const char* script, bool forceUserGesture, Node* thisObject)
{
    WebCore::Frame* coreFrame = m_private->core();
    if (!coreFrame) {
        return;
    ...
    coreFrame->script().executeUserAgentScriptInWorldIgnoringException(*world.priv().webcore(), WTF::String::fromUTF8(script), forceUserGesture, thisObject ? thisObject->priv().webcore() : nullptr);
}
