...
public:
    ...
    wkc_uint64 getElapsedTimeInMilliSeconds() const;
...
private:
    ...
    void notifyReceivedData(const uint8_t* data, size_t length);
...
public:
    ...
    virtual void didReceiveData(WebCore::ResourceHandle*, const WebCore::SharedBuffer&, int) override final;
    virtual void didFinishLoading(WebCore::ResourceHandle*, const WebCore::NetworkLoadMetrics&) override final;
...
WKCDownloadPrivate::setResponse(WebCore::ResourceHandle* in_handle, const WebCore::ResourceResponse& in_response)
{
    ...
    m_resourceHandle->setFrame(nullptr);
    m_resourceHandle->setMainFrame(nullptr, nullptr);
...
WKCDownloadPrivate::start()
{
    if (!m_resourceHandle) {
        ...
        if (!nctx)
            return false;
        m_resourceHandle = WebCore::ResourceHandle::create(nctx, m_request.priv().webcore(), m_client, false, false, false, nullptr, false);
        if (!m_resourceHandle) return false;
...
{
    wkc_uint64 cur = wkcGetTickCountPeer();
...
void
WKCDownloadPrivate::notifyReceivedData(const uint8_t* data, size_t length)
{
...
void
WKCDownloadClientPrivate::didReceiveData(WebCore::ResourceHandle*, const WebCore::SharedBuffer& data, int /*lengthReceived*/)
{
    m_download->notifyReceivedData(data.data(), data.size());
}

void
WKCDownloadClientPrivate::didFinishLoading(WebCore::ResourceHandle*, const WebCore::NetworkLoadMetrics&)
{
