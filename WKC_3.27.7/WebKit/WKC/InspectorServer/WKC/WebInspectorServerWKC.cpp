...
bool WebInspectorServer::platformResourceForPath(const WTF::String& path, Vector<uint8_t>& data, WTF::String& contentType)
{
    ...
    contentType = WebCore::MIMETypeRegistry::mimeTypeForExtension(ext);
    return true;
}

void WebInspectorServer::buildPageList(Vector<uint8_t>& data, WTF::String& contentType, bool jsonp)
{
    ...
    for (ClientMap::iterator it = m_clientMap.begin(); it != end; ++it) {
        ...
        builder.append(WTF::String(reinterpret_cast<const UChar*>(webView->title())));
        builder.append("\", \"url\": \"");
        builder.append(webView->uri());
        builder.append("\", \"inspectorUrl\": \"");
        builder.append("./Main.html?page=");
        builder.append(it->key);
        builder.append("\" }");
    }
    builder.append(" ]");
    ...
    data.append(cstr.dataAsUInt8Ptr(), cstr.length());
