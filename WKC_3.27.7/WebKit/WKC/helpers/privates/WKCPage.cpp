...
PagePrivate::backForwardList()
{
    ...
    WebCore::BackForwardClient* bfl = &m_webcore->backForward().client();
...
void
PagePrivate::setOpenedByDOM()
{
    if (!m_webcore)
        return;

    m_webcore->setOpenedByDOM();
}
...
PagePrivate::goToItem(HistoryItem* item, FrameLoadType type)
{
    ...
    m_webcore->goToItem(*item->priv().webcore(), toWebCoreFrameLoadType(type), WebCore::ShouldTreatAsContinuingLoad::No);
}
...
float
PagePrivate::deviceScaleFactor() const
{
    if (!m_webcore)
        return 1.0f;

    return m_webcore->deviceScaleFactor();
}
...
void
PagePrivate::updateRendering()
{
    if (!m_webcore)
        return;

    m_webcore->updateRendering();
}

void
PagePrivate::isolatedUpdateRendering()
{
    if (!m_webcore)
        return;

    m_webcore->isolatedUpdateRendering();
}

void
PagePrivate::finalizeRenderingUpdate()
{
    if (!m_webcore)
        return;

    m_webcore->finalizeRenderingUpdate({});
}

void
PagePrivate::accessibilitySettingsDidCange()
{
    m_webcore->accessibilitySettingsDidChange();
}
...
Page::setOpenedByDOM()
{
    return m_private.setOpenedByDOM();
}
...
float
Page::deviceScaleFactor() const
{
    return m_private.deviceScaleFactor();
}
...
{
    m_private.updateRendering();
}

void
Page::isolatedUpdateRendering()
{
    m_private.isolatedUpdateRendering();
}

void
Page::finalizeRenderingUpdate()
{
    m_private.finalizeRenderingUpdate();
}

void
Page::accessibilitySettingsDidCange()
{
    m_private.accessibilitySettingsDidCange();
}
