...
// backtrace
WKC_PEER_API int wkcDebugGetBacktracePeer(void** in_buffer, int in_size, int in_skip);
...
*/
WKC_PEER_API wkc_uint64 wkcGetTickCountPeer(void); // in ms
...
@param "in_thread" Pointer to thread object
@details
The version of wkcThreadEqualPeer() using SDK Private API.
@attention
in_thread must be a value obtained by wkcThreadCreatePeer().@n
This peer is called in:
@li SamplingProfiler::takeSample()

to suspend threads except the event happened.
*/
WKC_PEER_API void wkcThreadSuspendPeerEx(void* in_thread);
/**
...
@param "in_thread" Pointer to thread object
@details
The version of wkcThreadResumePeer() using SDK Private API.
@attention
in_thread must be a value obtained by wkcThreadCreatePeer().@n
This peer is called to the main thread suspended by wkcThreadSuspendPeerEx() in SamplingProfiler::takeSample().
*/
WKC_PEER_API void wkcThreadResumePeerEx(void* in_thread);
/**
...
struct WKCThreadContext_ {
    uint64_t fp;    //!< frame pointer
    uint64_t lr;    //!< link register
    uint64_t sp;    //!< stack pointer
    uint64_t pc;    //!< program counter
};
typedef struct WKCThreadContext_ WKCThreadContext;
...
/**
@brief Or variables shared by multiple threads
@param "in_val" Pointer to variables that should be or.
@param "in_op" variables that should be or.
@retval Or value
@details
Safely or the in_val variable shared by multiple threads.
*/
WKC_PEER_API uint64_t wkcThreadAtomicOrPeer(uint64_t volatile* in_val, uint64_t in_op);
/**
@brief Load variables shared by multiple threads
@param "in_val" Pointer to variables that not change
@retval Load value
@details
Safely load the in_val variable shared by multiple threads.
*/
WKC_PEER_API uint64_t wkcThreadAtomicLoadPeer(uint64_t volatile* in_val);
...
*/
WKC_PEER_API bool wkcThreadCheckAlivePeer(void);

enum {
    WKC_THREAD_FORCE_TERMINATE_FLAG_NONE        = 0x0,
    WKC_THREAD_FORCE_TERMINATE_FLAG_TRIGGERED   = 0x1,
    WKC_THREAD_FORCE_TERMINATE_FLAG_FINALIZING  = 0x2,
};
WKC_PEER_API void wkcThreadSetForceTerminateFlagPeer(int flag);

WKC_PEER_API bool wkcThreadIsForceTerminatingPeer(void);

WKC_PEER_API void wkcThreadSetMainThreadLongjmpProcPeer(void(*in_proc)());
...
#define WKC_MAX_THREADKEYS 40
typedef uint32_t WKCThreadKeyHandle;
...
*/
WKC_PEER_API int wkcThreadKeyNewPeer(WKCThreadKeyHandle* out_key, void(*in_destfunc)(void*));
/**
...
*/
WKC_PEER_API void wkcThreadKeyDeletePeer(WKCThreadKeyHandle in_key);
/**
...
*/
WKC_PEER_API void wkcThreadKeySetSpecificPeer(WKCThreadKeyHandle in_key, void* in_ptr);
/**
...
*/
WKC_PEER_API void* wkcThreadKeyGetSpecificPeer(WKCThreadKeyHandle in_key);
...
/**
   @brief Thread once control structure@n
 */
typedef char WKCOnce;

#define WKC_THREAD_ONCE_INIT (0)
...
@attention
- in_key will be registered implicity and reset on restart.
- The behavior must be the same as the pthread_once() function.
*/
WKC_PEER_API bool wkcThreadOncePeer(WKCOnce* in_key, void(*in_func)(void));
...
struct WKCLightCond_ {
    void* fCond;
};
typedef struct WKCLightCond_ WKCLightCond;
...
- The behavior must be the same as the pthread_cond_wait() function.
*/
WKC_PEER_API void wkcLightCondWaitPeer(WKCLightCond* in_cond, WKCLightMutex* in_mutex);
/**
...
@retval "== false" Failed
@details
In addition to behavior of the wkcLightCondWaitPeer() function, sets maximum waiting period.
@attention
- The behavior must be the same as the pthread_cond_timedwait() function.
*/
WKC_PEER_API bool wkcLightCondTimedWaitPeer(WKCLightCond* in_cond, WKCLightMutex* in_mutex, double in_offsettime);

WKC_PEER_API void wkcLightCondSignalPeer(WKCLightCond* in_cond);
WKC_PEER_API void wkcLightCondBroadcastPeer(WKCLightCond* in_cond);
WKC_PEER_API void wkcLightCondInitializePeer(WKCLightCond* in_cond);
WKC_PEER_API void wkcLightCondFinalizePeer(WKCLightCond* in_cond);
...
(TBD)
*/
WKC_PEER_API int wkcUnicodeUCharMemCaseCmpPeer(const U16Char* in_str1, const U16Char* in_str2, int in_len);
...
(TBD)
*/
WKC_PEER_API int wkcI18NEncodePeer(void* in_obj, const U16Char* in_str, unsigned int in_len, char* out_buf, unsigned int in_buflen, int* out_remains, int in_fallback);
...
(TBD)
*/
WKC_PEER_API int wkcI18NDecodePeer(void* in_obj, const char* in_str, unsigned int in_len, U16Char* out_buf, unsigned int in_buflen, int* out_remains, int in_stoponerror, int* out_error);
/**
@brief (TBD)
@param "in_idn" (TBD)
@param "in_len" (TBD)
...
(TBD)
*/
WKC_PEER_API int wkcI18NIDNfromUnicodePeer(const U16Char* in_idn, int in_len, unsigned char* out_host, int in_maxlen);
/**
@brief (TBD)
@param "in_host" (TBD)
@param "in_len" (TBD)
...
(TBD)
*/
WKC_PEER_API int wkcI18NIDNtoUnicodePeer(const unsigned char* in_host, int in_len, U16Char* out_idn, int in_maxlen);
...
(TBD)
*/
WKC_PEER_API bool wkcTextBreakIteratorSetStringPeer(void* in_self, const U16Char* in_str, int in_length);
/**
@brief (TBD)
@param "in_self" (TBD)
@retval (TBD)
...
(TBD)
*/
WKC_PEER_API bool wkcTextBreakIteratorSetPriorContextPeer(void* in_self, const U16Char* in_prior, int in_len);
...
*/
WKC_PEER_API const U16Char* wkcSystemGetNavigatorPlatformPeer(void);
/**
...
*/
WKC_PEER_API const U16Char* wkcSystemGetNavigatorProductPeer(void);
/**
...
*/
WKC_PEER_API const U16Char* wkcSystemGetNavigatorProductSubPeer(void);
/**
...
*/
WKC_PEER_API const U16Char* wkcSystemGetNavigatorVendorPeer(void);
/**
...
*/
WKC_PEER_API const U16Char* wkcSystemGetNavigatorVendorSubPeer(void);
/**
...
*/
WKC_PEER_API const U16Char* wkcSystemGetLanguagePeer(void);
/**
...
*/
WKC_PEER_API const U16Char* wkcSystemGetButtonLabelSubmitPeer(void);
/**
...
*/
WKC_PEER_API const U16Char* wkcSystemGetButtonLabelResetPeer(void);
/**
...
*/
WKC_PEER_API const U16Char* wkcSystemGetButtonLabelFilePeer(void);
/**
...
*/
WKC_PEER_API void wkcSystemSetNavigatorPlatformPeer(const U16Char* in_platform);
/**
...
*/
WKC_PEER_API void wkcSystemSetNavigatorProductPeer(const U16Char* in_product);
/**
...
*/
WKC_PEER_API void wkcSystemSetNavigatorProductSubPeer(const U16Char* in_product_sub);
/**
...
*/
WKC_PEER_API void wkcSystemSetNavigatorVendorPeer(const U16Char* in_vendor);
/**
...
*/
WKC_PEER_API void wkcSystemSetNavigatorVendorSubPeer(const U16Char* in_vendor_sub);
/**
...
*/
WKC_PEER_API void wkcSystemSetLanguagePeer(const U16Char* in_language);
/**
...
*/
WKC_PEER_API void wkcSystemSetButtonLabelSubmitPeer(const U16Char* in_label_submit);
/**
...
*/
WKC_PEER_API void wkcSystemSetButtonLabelResetPeer(const U16Char* in_label_reset);
/**
...
*/
WKC_PEER_API void wkcSystemSetButtonLabelFilePeer(const U16Char* in_label_file);
/**
...
WKC_PEER_API bool wkcCryptographicallyRandomValuesPeer(unsigned char* out_buf, size_t in_buflen);
