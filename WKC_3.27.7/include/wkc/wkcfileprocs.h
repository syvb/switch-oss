...
/**
@brief Callback function for getting integer file description, set by wkcFileCallbackSetPeer()
@param "in_fd" File descriptor
@retval ">= 0" Succeeded
@retval "== -1" Failed
@details
Returns the integer file descriptor associated with in_fd.@n
The behavior must be the same as the POSIX fileno() function.
*/
typedef int    (*wkcFileFilenoProc)(void* in_fd);
/**
...
struct WKCFileProcs_ {
    ...
    /** @brief Callback function for getting integer file descriptor */
    wkcFileFilenoProc fFilenoProc;
