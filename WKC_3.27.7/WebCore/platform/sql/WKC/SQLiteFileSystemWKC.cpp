...
wkcSync(sqlite3_file *id, int flags)
{
    ...
    file = (wkcFile*)id;
    int fd = wkcFileFilenoPeer(file->fFile);
    if (fd == -1) {
        file->fLastError = wkcFileErrnoPeer();
        return SQLITE_IOERR_FSYNC;
    }
    err = wkcFileFSyncPeer(fd);
    if (err != 0) {
        file->fLastError = wkcFileErrnoPeer();
    }
...
wkcDelete(sqlite3_vfs*, const char *zName, int)
{
    ...
    return (err == 0) ? SQLITE_OK : SQLITE_IOERR_DELETE;
}
