...
{
    WTF::Vector<String> values = value.split(';');
...
void setCookiesFromDOM(const NetworkStorageSession& session, const URL& firstParty, const SameSiteInfo&, const URL& url, std::optional<FrameIdentifier> frameID, std::optional<PageIdentifier> pageID, const String& value)
{
    ...
    URL opt_url = url.truncatedForUseAsBase();
    ResourceHandleManager::sharedInstance()->setCookie(opt_url.host().toString(), opt_url.path().toString(), value);
}
...
std::pair<String, bool> cookiesInternal(const NetworkStorageSession& session, const URL& firstParty, const URL& url, bool includehttponly)
{
    ...
    return { ResourceHandleManager::sharedInstance()->getCookie(opt_url.host().toString(), opt_url.path().toString(), secure, includehttponly), false };
}

std::pair<String, bool> cookiesForDOM(const NetworkStorageSession& session, const URL& firstParty, const SameSiteInfo&, const URL& url, std::optional<FrameIdentifier> frameID, std::optional<PageIdentifier> pageID, IncludeSecureCookies)
{
...
std::pair<String, bool> cookieRequestHeaderFieldValue(const NetworkStorageSession& session, const URL& firstParty, const SameSiteInfo&, const URL& url, std::optional<FrameIdentifier> frameID, std::optional<PageIdentifier> pageID, IncludeSecureCookies)
{
...
bool getRawCookies(const NetworkStorageSession&, const URL& firstParty, const SameSiteInfo&, const URL& url, std::optional<FrameIdentifier> frameID, std::optional<PageIdentifier> pageID, Vector<Cookie>& rawCookies)
{
    return ResourceHandleManager::sharedInstance()->getRawCookies(url.host().toString(), url.path().toString(), url.protocolIs("https"), rawCookies);
}

void deleteCookie(const NetworkStorageSession&, const URL& url, const String& name)
{
    ResourceHandleManager::sharedInstance()->deleteCookie(url.host().toString(), url.path().toString(), url.protocolIs("https"), name);
}
