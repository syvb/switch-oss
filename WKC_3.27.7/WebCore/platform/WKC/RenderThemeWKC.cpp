...
public:
    ...
    virtual void paintSearchFieldDecorations(const RenderBox&, const PaintInfo&, const IntRect&) override;
    ...
    virtual Color platformFocusRingColor(OptionSet<StyleColorOptions>) const override;

    virtual Color systemColor(CSSValueID cssValueId, OptionSet<StyleColorOptions>) const override;

    virtual Color platformActiveSelectionBackgroundColor(OptionSet<StyleColorOptions>) const override;
    virtual Color platformInactiveSelectionBackgroundColor(OptionSet<StyleColorOptions>) const override;

    virtual Color platformActiveSelectionForegroundColor(OptionSet<StyleColorOptions>) const override;
    virtual Color platformInactiveSelectionForegroundColor(OptionSet<StyleColorOptions>) const override;

    virtual LengthBox popupInternalPaddingBox(const RenderStyle&, const Settings&) const override;
...
static SRGBA<uint8_t> toSRGBA(unsigned source)
{
    return asSRGBA(PackedColor::ARGB { source });
}
...
Color RenderThemeWKC::systemColor(CSSValueID cssValueId, OptionSet<StyleColorOptions> options) const
{
    int id = 0;
    ...
    case CSSValueWebkitActivelink:
        id = WKC_SKINCOLOR_ACTIVETEXT; break;
    ...
    case CSSValueCanvas:
        id = WKC_SKINCOLOR_CANVAS; break;
    case CSSValueCanvastext:
        id = WKC_SKINCOLOR_CANVASTEXT; break;
    case CSSValueCaptiontext:
        id = WKC_SKINCOLOR_CAPTIONTEXT; break;
    case CSSValueField:
        id = WKC_SKINCOLOR_FIELD; break;
    case CSSValueFieldtext:
        id = WKC_SKINCOLOR_FIELDTEXT; break;
    ...
    case CSSValueLinktext:
        id = WKC_SKINCOLOR_LINKTEXT; break;
    case CSSValueWebkitLink:
        id = forVisitedLink ? WKC_SKINCOLOR_VISITEDTEXT : WKC_SKINCOLOR_LINKTEXT; break;
    case CSSValueMenu:
    ...
    return toSRGBA(wkcStockImageGetSkinColorPeer(id, useDarkAppearance));
}

Color RenderThemeWKC::platformFocusRingColor(OptionSet<StyleColorOptions> options) const
{
    const bool useDarkAppearance = options.contains(StyleColorOptions::UseDarkAppearance);
    return toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_FOCUSRING, useDarkAppearance));
}
...
void RenderThemeWKC::setCheckboxSize(RenderStyle& style) const
{
    ...
    if (style.width().isIntrinsicOrAuto())
        style.setWidth(Length(fw, LengthType::Fixed));

    if (style.height().isAuto())
        style.setHeight(Length(fw, LengthType::Fixed));
}
...
static void
calcSkinRect(unsigned int in_skinWidth, unsigned int in_skinHeight, const FloatRect& in_r, WKCFloatRect& out_dest)
{
...
bool RenderThemeWKC::paintCheckbox(const RenderObject& o, const PaintInfo& i, const FloatRect& r)
{
    ...
    index = 0;
    if (this->isEnabled(o)) {
        if (this->isFocused(o) || this->isHovered(o))
    ...
    image_buf = wkcStockImageGetBitmapPeer (index, o.useDarkAppearance());
    if (!image_buf)
          return false;
...
void RenderThemeWKC::adjustTextFieldStyle(RenderStyle& style, const Element* e) const
{
    if (style.hasBackgroundImage()) {
        style.resetBorder();
        const bool useDarkAppearance = (e && e->renderer()) ? e->renderer()->useDarkAppearance() : false;
        const unsigned int defaultBorderColor = wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_TEXTFIELD_BORDER, useDarkAppearance);
...
bool RenderThemeWKC::paintTextField(const RenderObject& o, const PaintInfo& i, const FloatRect& r)
{
    ...
    const bool useDarkAppearance = o.useDarkAppearance();
    const Color defaultBorderColor(toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_TEXTFIELD_BORDER, useDarkAppearance)));
    bool ret = false;
    ...
    if (o.style().hasBackground()) {
        ...
        backgroundColor = Color(toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_TEXTFIELD_BACKGROUND, useDarkAppearance)));
    }
...
void RenderThemeWKC::paintSearchFieldDecorations(const RenderBox&, const PaintInfo&, const IntRect&)
{
}
...
bool RenderThemeWKC::paintMenuList(const RenderObject& o, const PaintInfo& i, const FloatRect& r)
{
    ...
    paintMenuListButtonDecorations(downcast<RenderBox>(o), i, r);
    return false;
}
...
void RenderThemeWKC::paintMenuListButtonDecorations(const RenderBox& o, const PaintInfo& i, const FloatRect& r)
{
    void *drawContext;
    WKCFloatRect src, dest;
    int index;
...
bool RenderThemeWKC::paintSliderTrack(const RenderObject& o, const PaintInfo& i, const IntRect& r)
{
    ...
    Color backgroundColor;
    const bool useDarkAppearance = o.useDarkAppearance();
    const Color defaultBorderColor(toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_RANGE_BORDER, useDarkAppearance)));
    ...
    _setBorder(i.context(), defaultBorderColor, 1.0);
    if (!this->isEnabled(o)) {
        backgroundColor = Color(toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_RANGE_BACKGROUND_DISABLED, useDarkAppearance)));
    } else {
        backgroundColor = o.style().visitedDependentColor(CSSPropertyBackgroundColor);
    }
    i.context().setFillColor(backgroundColor);
...
{
    int index = 0;
    if (style.effectiveAppearance() == SliderThumbHorizontalPart) {
        index = WKC_IMAGE_H_RANGE;
    } else if (style.effectiveAppearance() == SliderThumbVerticalPart) {
...
bool RenderThemeWKC::paintSliderThumb(const RenderObject& o, const PaintInfo& i, const IntRect& r)
{
    ...
    index = 0;
    if (o.style().effectiveAppearance() == SliderThumbHorizontalPart) {
        if (this->isEnabled(o)) {
            index = WKC_IMAGE_H_RANGE;
            if (this->isHovered(o) || this->isFocused(o))
    ...
    } else if (o.style().effectiveAppearance() == SliderThumbVerticalPart) {
        if (this->isEnabled(o)) {
            index = WKC_IMAGE_V_RANGE;
            if (this->isHovered(o) || this->isFocused(o))
...
void RenderThemeWKC::adjustSliderThumbSize(RenderStyle& style, const Element*) const
{
    ...
    style.setWidth(Length((float)width*scale, LengthType::Fixed));
    style.setHeight(Length((float)height*scale, LengthType::Fixed));
}

Color RenderThemeWKC::platformActiveSelectionBackgroundColor(OptionSet<StyleColorOptions> options) const
{
    const bool useDarkAppearance = options.contains(StyleColorOptions::UseDarkAppearance);
    return toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_ACTIVESELECTIONBACKGROUND, useDarkAppearance));
}

Color RenderThemeWKC::platformInactiveSelectionBackgroundColor(OptionSet<StyleColorOptions> options) const
{
    const bool useDarkAppearance = options.contains(StyleColorOptions::UseDarkAppearance);
    return toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_INACTIVESELECTIONBACKGROUND, useDarkAppearance));
}

Color RenderThemeWKC::platformActiveSelectionForegroundColor(OptionSet<StyleColorOptions> options) const
{
    const bool useDarkAppearance = options.contains(StyleColorOptions::UseDarkAppearance);
    return toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_ACTIVESELECTIONFOREGROUND, useDarkAppearance));
}

Color RenderThemeWKC::platformInactiveSelectionForegroundColor(OptionSet<StyleColorOptions> options) const
{
    const bool useDarkAppearance = options.contains(StyleColorOptions::UseDarkAppearance);
    return toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_INACTIVESELECTIONFOREGROUND, useDarkAppearance));
}

LengthBox RenderThemeWKC::popupInternalPaddingBox(const RenderStyle& style, const Settings&) const
{
    int leftWidth = 0;
    int rightWidth = 0;
...
String RenderThemeWKC::fileListNameForWidth(const FileList* fileList, const FontCascade& fc, int width, bool multipleFilesAllowed) const
{
    ...
    String str;
    UChar filenameBuffer[MAX_PATH] = {0};
    ...
    if (1 < fileList->length()) {
        ...
        if (gResolveFilenameForDisplayProc != 0) {
            ...
            (*gResolveFilenameForDisplayProc)(reinterpret_cast<unsigned short*>(file.charactersWithNullTermination().data()), cur_len, reinterpret_cast<unsigned short*>(filenameBuffer), &len, MAX_PATH);
...
Vector<String, 2> RenderThemeWKC::mediaControlsScripts()
{
...
Seconds RenderThemeWKC::animationRepeatIntervalForProgressBar(const RenderProgress& p) const
{
    return gFrameRateForProgressBarAnimation;
}

Seconds RenderThemeWKC::animationDurationForProgressBar(const RenderProgress& p) const
{
    if (p.isDeterminate())
        return gFrameRateForProgressBarAnimation;
...
bool RenderThemeWKC::paintProgressBar(const RenderObject& o, const PaintInfo& i, const IntRect& r)
{
    ...
    Color backgroundColor;
    const bool useDarkAppearance = o.useDarkAppearance();
    const Color defaultBorderColor(toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_PROGRESSBAR_BORDER, useDarkAppearance)));
    const Color bodyColor(toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_PROGRESSBAR_BODY, useDarkAppearance)));

    i.context().save();

    _setBorder(i.context(), defaultBorderColor, 1.0);
    if (!this->isEnabled(o)) {
        backgroundColor = Color(toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_PROGRESSBAR_BACKGROUND_DISABLED, useDarkAppearance)));
    } else if (o.style().hasBackground()) {
        backgroundColor = o.style().visitedDependentColor(CSSPropertyBackgroundColor);
    } else {
        backgroundColor = Color(toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_PROGRESSBAR_BACKGROUND, useDarkAppearance)));
    }
    if (!o.style().hasBackgroundImage()) {
        i.context().setFillColor(backgroundColor);
        i.context().drawRect(r);
...
static bool _paintSpinButton(const RenderObject& o, const PaintInfo& i, const IntRect& r, int index, bool up)
{
    ...
    Color c(toSRGBA(wkcStockImageGetSkinColorPeer(WKC_SKINCOLOR_BUTTONTEXT, o.useDarkAppearance())));
    i.context().save();
