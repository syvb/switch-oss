...
{
  // nothing to do on WKC.
}
#endif

bool isFastMallocEnabled()
{
    // in WKC environment, FastMalloc is always enabled.
    return true;
}
...
void* tryFastAlignedMalloc(size_t align, size_t size)
{
    return wkcHeapMallocAlignedPeer(align, size, true, false);
}
...
{
#if 0
    if (!wkcMemoryCommitPeer(ptr, size, true, false))
        MCRASH(size);
#endif
}
...
{
#if 0
    wkcMemoryDecommitPeer(ptr, size);
#endif
}
...
voidFuncAllocator()
{
    WKC_DEFINE_STATIC_TYPE(WKCAllocator<std::function<void()>>*, allocator, new WKCAllocator<std::function<void()>>());
    return *allocator;
}
...
{
    WKC_DEFINE_STATIC_TYPE(WKCAllocator<std::function<void(WebCore::ScriptExecutionContext&)>>*, allocator, new WKCAllocator<std::function<void(WebCore::ScriptExecutionContext&)>>());
    return *allocator;
}
