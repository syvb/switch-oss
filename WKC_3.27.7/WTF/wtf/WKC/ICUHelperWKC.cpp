...
#if 0 // defined in ubidi_props.cpp
UCharDirection
...
{
    return wkcUnicodeUCharMemCaseCmpPeer(s1, s2, length);
}
...
{
    *status = U_ZERO_ERROR;
    int len = wkcI18NIDNfromUnicodePeer(src, srcLength, 0, 0);
    if (len<=0) {
    ...
    ::memset(d, 0, len+1);
    len = wkcI18NIDNfromUnicodePeer(src, srcLength, d, len+1);
    if (len<0) {
        WTF::fastFree((void *)d);
        *status = U_ILLEGAL_ARGUMENT_ERROR;
        return 0;
...
u_hasBinaryProperty(UChar32 c, UProperty which)
{
    ...
    case UCHAR_ID_CONTINUE:
        return U_GET_GC_MASK(c) & (U_GC_L_MASK | U_GC_MN_MASK | U_GC_MC_MASK | U_GC_ND_MASK | U_GC_PC_MASK);
    case UCHAR_ID_START:
        return U_GET_GC_MASK(c) & (U_GC_L_MASK | U_GC_NL_MASK);
