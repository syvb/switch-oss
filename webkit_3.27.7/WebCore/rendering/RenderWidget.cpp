...
#else
    WKC_DEFINE_STATIC_HASHMAP_PTR(const Widget*, RenderWidget*, staticWidgetRendererMap, 0);
...
#else
WKC_DEFINE_GLOBAL_CLASS_OBJ_ZERO(unsigned, WidgetHierarchyUpdatesSuspensionScope, s_widgetHierarchyUpdateSuspendCount);
WKC_DEFINE_GLOBAL_CLASS_OBJ_ZERO(bool, WidgetHierarchyUpdatesSuspensionScope, s_haveScheduledWidgetToMove);
#endif
...
#else
    if (visibleToHitTesting() && shouldHitTestChildFrameContent && hasRenderView) {
#endif
