...
#if PLATFORM(WKC)
void RenderInline::addFocusRingRects(Vector<LayoutRect>& rects, const LayoutPoint& additionalOffset, const RenderLayerModelObject* paintContainer, bool useTransform)
#else
...
#endif
{
    ...
    if (useTransform) {
        Vector<LayoutRect> tmpRects;
        LayoutPoint point;
        AbsoluteRectsGeneratorContext context(tmpRects, point);
        generateLineBoxRects(context);
        // Add transformed rects.
        for (auto tmpRect : tmpRects)
            rects.append(roundedIntRect(localToContainerQuad(FloatRect(tmpRect), paintContainer).boundingBox()));
    } else {
    ...
    for (auto& child : childrenOfType<RenderElement>(*this)) {
        ...
        child.addFocusRingRects(rects, flooredIntPoint(pos), paintContainer, useTransform);
#endif
    }
...
#else
        if (continuation->isInline())
            continuation->addFocusRingRects(rects, flooredLayoutPoint(LayoutPoint(additionalOffset + continuation->containingBlock()->location() - containingBlock()->location())), paintContainer, useTransform);
        else
            continuation->addFocusRingRects(rects, flooredLayoutPoint(LayoutPoint(additionalOffset + downcast<RenderBox>(*continuation).location() - containingBlock()->location())), paintContainer, useTransform);
#endif
    }
}
