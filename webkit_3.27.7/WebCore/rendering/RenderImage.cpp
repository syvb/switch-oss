...
#else
            float scale = Image::platformResourceScale();
            if (shouldDisplayBrokenImageIcon() && !image->isNull() && usableSize.width() >= image->width() * scale && usableSize.height() >= image->height() * scale) {
#endif
