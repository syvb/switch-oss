...
#else
WKC_DEFINE_GLOBAL_CLASS_OBJ_ZERO(bool, RenderBlock, s_canPropagateFloatIntoSibling);
#endif
...
#else
void RenderBlockFlow::addFocusRingRectsForInlineChildren(Vector<LayoutRect>& rects, const LayoutPoint& additionalOffset, const RenderLayerModelObject* paintContainer, bool useTransform)
#endif
{
    ASSERT(childrenInline());
    for (auto box = InlineIterator::firstRootInlineBoxFor(*this); box; box.traverseNextInlineBox()) {
        ...
        if (useTransform) {
            ...
            FloatRect rect(box->rect().x(), top, box->rect().width(), bottom - top);
            if (!rect.isEmpty())
                rects.append(roundedIntRect(localToContainerQuad(rect, paintContainer).boundingBox()));
        } else {
            LayoutRect rect(additionalOffset.x() + box->rect().x(), additionalOffset.y() + top, box->rect().width(), bottom - top);
            if (!rect.isEmpty())
                rects.append(rect);
        }
#endif
