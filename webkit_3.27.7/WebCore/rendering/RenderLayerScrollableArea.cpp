...
#if PLATFORM(WKC)
        if (resizeCornerImageHiRes.isNull())
            resizeCornerImageHiRes.construct(&Image::loadPlatformResource("textAreaResizeCorner@2x").leakRef());
#endif
...
#if PLATFORM(WKC)
        if (resizeCornerImageLoRes.isNull())
            resizeCornerImageLoRes.construct(&Image::loadPlatformResource("textAreaResizeCorner").leakRef());
#endif
...
#else
    WKC_DEFINE_STATIC_TYPE_WITH_DEFAULT_CONSTRUCT(IntPoint, previousMousePosition);
#endif
