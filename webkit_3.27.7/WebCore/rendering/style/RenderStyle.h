...
// attribute setter methods
...
#if ENABLE(CSS_IMAGE_RESOLUTION)
#if PLATFORM(WKC)
    void setImageResolutionSource(ImageResolutionSource v) { SET_VAR(m_rareInheritedData, imageResolutionSource, static_cast<unsigned>(v)); }
    void setImageResolutionSnap(ImageResolutionSnap v) { SET_VAR(m_rareInheritedData, imageResolutionSnap, static_cast<unsigned>(v)); }
#else
    ...
    static const TransformOperations& initialTransform() {
        static NeverDestroyed<TransformOperations> ops;
        if (ops.isNull())
            ops.construct();
        return ops;
    }
    ...
    static const FilterOperations& initialFilter() {
        static NeverDestroyed<FilterOperations> ops;
        if (ops.isNull())
            ops.construct();
        return ops;
    }
    static const FilterOperations& initialAppleColorFilter() {
        static NeverDestroyed<FilterOperations> ops;
        if (ops.isNull())
            ops.construct();
        return ops;
