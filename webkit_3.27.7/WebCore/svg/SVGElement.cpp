...
#if PLATFORM(WKC)
        if (emptyInstances.isNull())
            emptyInstances.construct();
#endif
...
#if PLATFORM(WKC)
    if (set.isNull())
        set.construct(createSVGLayerAwareElementSet());
#endif
...
#else
    static LazyNeverDestroyed<MemoryCompactLookupOnlyRobinHoodHashMap<AtomString, QualifiedName>> animatableAttributes;
    if (animatableAttributes.isNull())
        animatableAttributes.construct([](){
#endif
...
CSSPropertyID SVGElement::cssPropertyIdForSVGAttributeName(const QualifiedName& attrName)
{
    ...
    if (properties.isNull())
        properties.construct(createAttributeNameToCSSPropertyIDMap());
#endif
