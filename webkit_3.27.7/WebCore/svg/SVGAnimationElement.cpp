...
#else
    static LazyNeverDestroyed<MemoryCompactLookupOnlyRobinHoodHashSet<QualifiedName>> supportedAttributes;
    if (supportedAttributes.isNull())
        supportedAttributes.construct([](){
#endif
...
#if PLATFORM(WKC)
    if (css.isNull())
        css.construct("CSS", AtomString::ConstructFromLiteral);
#endif
    static MainThreadNeverDestroyed<const AtomString> xml("XML", AtomString::ConstructFromLiteral);
#if PLATFORM(WKC)
    if (xml.isNull())
        xml.construct("XML", AtomString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC)
    if (sum.isNull())
        sum.construct("sum", AtomString::ConstructFromLiteral);
#endif
