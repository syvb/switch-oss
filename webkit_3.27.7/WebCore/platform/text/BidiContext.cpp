...
#if PLATFORM(WKC)
            if (ltrContext.isNull())
                ltrContext.construct();
#endif
            static WKCStd::once_flag ltrContextOnceFlag;
            WKCStd::call_once(ltrContextOnceFlag, [&]() {
...
#if PLATFORM(WKC)
        if (ltrOverrideContext.isNull())
            ltrOverrideContext.construct();
#endif
        static WKCStd::once_flag ltrOverrideContextOnceFlag;
        WKCStd::call_once(ltrOverrideContextOnceFlag, [&]() {
...
#if PLATFORM(WKC)
        if (rtlContext.isNull())
            rtlContext.construct();
#endif
        static WKCStd::once_flag rtlContextOnceFlag;
        WKCStd::call_once(rtlContextOnceFlag, [&]() {
...
#if PLATFORM(WKC)
    if (rtlOverrideContext.isNull())
        rtlOverrideContext.construct();
#endif
    static WKCStd::once_flag rtlOverrideContextOnceFlag;
    WKCStd::call_once(rtlOverrideContextOnceFlag, [&]() {
