...
#if PLATFORM(WKC)
bool isCombining(UChar32 character, UChar32 previousCharacter)
{
    // this is a function to judge whether the combining character is proper for previousCharacter
    ...
    if (character == 0x3099) {
        ...
        for (int i = 0; specials[i]; i++) {
            if (previousCharacter == specials[i])
                return true;
        }

        if (previousCharacter >= 0x304B && previousCharacter <= 0x3061 && previousCharacter % 2 == 1)
            return true;
        if (previousCharacter >= 0x3064 && previousCharacter <= 0x3068 && previousCharacter % 2 == 0)
            return true;
        if (previousCharacter >= 0x306F && previousCharacter <= 0x307D && previousCharacter % 3 == 0)
            return true;
        if (previousCharacter >= 0x30AB && previousCharacter <= 0x30C1 && previousCharacter % 2 == 1)
            return true;
        if (previousCharacter >= 0x30C4 && previousCharacter <= 0x30C8 && previousCharacter % 2 == 0)
            return true;
        if (previousCharacter >= 0x30CF && previousCharacter <= 0x30DB && previousCharacter % 3 == 0)
            return true;
    } else if (character == 0x309A) {
        if (previousCharacter >= 0x306F && previousCharacter <= 0x307D && previousCharacter % 3 == 0)
            return true;
        if (previousCharacter >= 0x30CF && previousCharacter <= 0x30DB && previousCharacter % 3 == 0)
            return true;
...
#if PLATFORM(WKC)
    LONG_LOOP_DETECTION_SCOPE;
#endif
    while (textIterator.consume(character, clusterLength)) {
#if PLATFORM(WKC)
        CRASH_IF_LONG_LOOP_DETECTED();
#endif
...
#if PLATFORM(WKC)
            if (currentCharacterIndex && isCombining(character, m_run.text()[currentCharacterIndex - 1]))
                width = 0.0f;
#endif
