...
#if PLATFORM(WKC)
        if (noReferrer.isNull())
            noReferrer.construct("noreferrer", AtomString::ConstructFromLiteral);
#endif
        static MainThreadNeverDestroyed<const AtomString> noOpener("noopener", AtomString::ConstructFromLiteral);
#if PLATFORM(WKC)
        if (noOpener.isNull())
            noOpener.construct("noopener", AtomString::ConstructFromLiteral);
#endif
        static MainThreadNeverDestroyed<const AtomString> opener("opener", AtomString::ConstructFromLiteral);
#if PLATFORM(WKC)
        if (opener.isNull())
            opener.construct("opener", AtomString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC)
    if (systemPreviewRelValue.isNull())
        systemPreviewRelValue.construct("ar", AtomString::ConstructFromLiteral);
#endif
