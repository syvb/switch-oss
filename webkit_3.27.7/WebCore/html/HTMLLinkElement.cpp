...
#if PLATFORM(WKC)
    if (sharedLoadEventSender.isNull())
        sharedLoadEventSender.construct(eventNames().loadEvent);
#endif
...
#if PLATFORM(WKC)
    if (sharedErrorEventSender.isNull())
        sharedErrorEventSender.construct(eventNames().errorEvent);
#endif
