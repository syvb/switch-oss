...
#if PLATFORM(WKC)
    if (twoPowerOfFloatMantissaBits.isNull())
        twoPowerOfFloatMantissaBits.construct(Decimal::Positive, 0, UINT64_C(1) << FLT_MANT_DIG);
#endif
...
#if PLATFORM(WKC)
    if (tenPowerOf21.isNull())
        tenPowerOf21.construct(Decimal::Positive, 21, 1);
#endif
...
#if PLATFORM(WKC)
    if (twoPowerOfDoubleMantissaBits.isNull())
        twoPowerOfDoubleMantissaBits.construct(Decimal::Positive, 0, UINT64_C(1) << DBL_MANT_DIG);
#endif
