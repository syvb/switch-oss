...
#if PLATFORM(WKC)
    if (selectMultiple.isNull())
        selectMultiple.construct("select-multiple", AtomString::ConstructFromLiteral);
#endif
    static MainThreadNeverDestroyed<const AtomString> selectOne("select-one", AtomString::ConstructFromLiteral);
#if PLATFORM(WKC)
    if (selectOne.isNull())
        selectOne.construct("select-one", AtomString::ConstructFromLiteral);
#endif
...
HTMLSelectElement::showPopup()
{
    ...
    if (RenderMenuList* menuList = downcast<RenderMenuList>(renderer())){
        if (menuList->popupIsVisible()){
            // If this popup is already opened, hide it once.
            menuList->hidePopup();
        }
        saveLastSelection();
        menuList->showPopup();
