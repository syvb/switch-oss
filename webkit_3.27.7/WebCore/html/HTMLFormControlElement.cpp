...
#else
    static LazyNeverDestroyed<QualifiedName> nxDisabledFocusableAttr;
    if (nxDisabledFocusableAttr.isNull())
        nxDisabledFocusableAttr.construct(QualifiedName(nullAtom(), "nx-disabled-focusable", nullAtom()));
    return !isDisabledFormControl() || hasAttributeWithoutSynchronization(nxDisabledFocusableAttr);
#endif
}
