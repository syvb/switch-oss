...
#if PLATFORM(WKC)
        if (brokenImageVeryHiRes.isNull())
            brokenImageVeryHiRes.construct(&Image::loadPlatformResource("missingImage@3x").leakRef());
        return std::make_pair(brokenImageVeryHiRes.get(), 3);
#else
...
#if PLATFORM(WKC)
        if (brokenImageHiRes.isNull())
            brokenImageHiRes.construct(&Image::loadPlatformResource("missingImage@2x").leakRef());
...
#if PLATFORM(WKC)
    if (brokenImageLoRes.isNull())
        brokenImageLoRes.construct(&Image::loadPlatformResource("missingImage").leakRef());
    float scale = 1.f / Image::platformResourceScale();
    return std::make_pair(brokenImageLoRes.get(), scale);
#else
