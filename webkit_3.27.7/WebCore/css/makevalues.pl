...
#else
    static LazyNeverDestroyed<AtomString*> valueKeywordStrings;
    if (valueKeywordStrings.isNull())
        valueKeywordStrings.construct(new AtomString[numCSSValueKeywords]);
    AtomString& valueKeywordString = (valueKeywordStrings.get())[id];
#endif
