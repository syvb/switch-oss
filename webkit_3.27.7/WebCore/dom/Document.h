...
public:
    ...
    void processInitialAutofocus(const String& value);
    void disallowLayout(bool disallow) { m_disallowLayout = disallow; }
#endif
    ...
    void setTouchEventRegionsNeedUpdate() { m_touchEventRegionsNeedUpdate = true; }
    void updateTouchEventRegions();
    void updateTouchEventRegionsForFrame();
...
#else
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY_ZERO(uint64_t, s_globalTreeVersion);
#endif
...
#endif
#if PLATFORM(WKC) && ENABLE(TOUCH_EVENTS)
    bool m_touchEventRegionsNeedUpdate { false };
...
#if PLATFORM(WKC)
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY(bool, hasEverCreatedAnAXObjectCache);
#else
