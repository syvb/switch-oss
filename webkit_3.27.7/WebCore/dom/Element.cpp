...
{
#if PLATFORM(WKC)
    if (isScrollableOverFlowBlockNode())
        return 0;
    if (hasEditableStyle() && parentNode() && !parentNode()->hasEditableStyle())
        return 0;
#endif
    return -1;
}
...
void Element::setHasFocusVisible(bool value)
{
    ...
    if (focused()) {
        if (document().page()) {
            value = document().page()->chrome().client().dispatchWillSetHasFocusVisibleForFocusedElement(*this, value);
        }
    }
    ...
    Style::PseudoClassChangeInvalidation focusStyleInvalidation(*this, CSSSelector::PseudoClassFocusVisible, value);
#endif
...
#if PLATFORM(WKC)
        else if (name == HTMLNames::loopAttr) {
            if (is<HTMLMediaElement>(*this))
                downcast<HTMLMediaElement>(*this).setLoop(newValue != nullAtom());
        }
#endif
    }
...
static bool canAttachAuthorShadowRoot(const Element& element)
{
...
#else
    static LazyNeverDestroyed<MemoryCompactLookupOnlyRobinHoodHashSet<AtomString>> tagNames;
    auto initializer = [] {
#endif
    ...
    if(tagNames.isNull())
        tagNames.construct(initializer());
#endif
...
#endif
#if PLATFORM(WKC)
    if (gProhibitsScrollingEnabled)
        revealMode = SelectionRevealMode::RevealUpToFrameOrScrollsOverflow;
#endif
