...
#else
WKC_DEFINE_GLOBAL_BOOL(shouldIgnoreLeaks, false);
#endif
...
#if PLATFORM(WKC)
static Element* parentOrShadowHostOrFrameOwnerElement(const Node* node)
{
...
static LayoutRect intersectWithContainerExcludingEdge(LayoutRect rect, LayoutRect container)
{
    container.move(1, 1);
    container.contract(2, 2);
    rect.intersect(container);
    return rect;
}

static void clipToEnclosingScrollableContentRect(const Node* node, LayoutRect& rect)
{
    if (!node || rect.isEmpty())
        return;

    while (node = parentOrShadowHostOrFrameOwnerElement(node)) {
        ...
        if (is<HTMLFrameOwnerElement>(node)) {
            const HTMLFrameOwnerElement* frameOwnerElement = downcast<HTMLFrameOwnerElement>(node);
            if (frameOwnerElement->contentFrame()) {
                Document* document = frameOwnerElement->contentFrame()->document();
                if (document && document->view()) {
                    rect = intersectWithContainerExcludingEdge(rect, document->view()->visibleContentRect());
                    return;
                }
            }
        }
        ...
        if (box->scrollsOverflow()) {
            IntRect r = snappedIntRect(box->clientBoxRect());
            r.moveBy(renderer->absoluteBoundingBoxRect(true).location());
            rect = intersectWithContainerExcludingEdge(rect, r);
            return;
        }
    }
}

void Node::getNodeCompositeRect(LayoutRect* rects, bool isOverflowXHidden, bool isOverflowYHidden, int tx, int ty) const
{
        ...
        if (o) {
            ...
            if (!rect.isEmpty() && !(rect.x() < 0 || rect.y() < 0)) {
                if (element->firstChild()) {
                    bool x = (o->style().overflowX() == Overflow::Hidden);
                    bool y = (o->style().overflowY() == Overflow::Hidden);
...
LayoutRect Node::absoluteCompositeTransformedRectInternal(bool isDescendant, bool fastPath) const
{
    ...
    if (fastPath) {
        ...
        if (hasTagName(HTMLNames::aTag) && firstChild()) {
            const RenderStyle& style = renderer()->style();
            bool isOverflowXHidden = (style.overflowX() == Overflow::Hidden);
            bool isOverflowYHidden = (style.overflowY() == Overflow::Hidden);
            getNodeCompositeRect(&rect, isOverflowXHidden, isOverflowYHidden);
        }
        return rect;
    }

    if (!renderer()->absoluteBoundingBoxRect(false).isEmpty()) {
        if (isDescendant) {
            rect = renderer()->absoluteClippedOverflowRectForRepaint();
            // Workaround for the problem that the rectangle of absoluteClippedOverflowRect() may be larger than expected.
            rect.intersect(renderer()->absoluteBoundingBoxRect(true));
        } else {
            rect = renderer()->absoluteBoundingBoxRect(true);
        }
    }
    ...
    for (Element* element = ElementTraversal::firstWithin((Element &)*this); element; element = ElementTraversal::nextSibling(*element)) {
        if (!element->renderer())
            continue;
        if (!rect.isEmpty()) {
            if (element->renderer()->isFixedPositioned() || element->renderer()->isAbsolutelyPositioned())
                continue;
        }
        rect.unite(element->absoluteCompositeTransformedRectInternal(true, false));
    }

    return rect;
}

LayoutRect Node::absoluteCompositeTransformedRect(bool ignoreBorder, bool clip, bool fastPath) const
{
    LayoutRect rect = absoluteCompositeTransformedRectInternal(false, fastPath);
    ...
    if (clip)
        clipToEnclosingScrollableContentRect(this, rect);

    return rect;
}

bool Node::isScrollableOverFlowBlockNode() const
{
    if (!renderer() || renderer()->style().visibility() != Visibility::Visible)
        return false;
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_TYPE_DEFAULT_ZERO(Lock, s_eventTargetDataMapLock);
#else
