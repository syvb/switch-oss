...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_UINT_ZERO(gNonUtilityPageCount);
#else
...
#else
    WKC_DEFINE_STATIC_BOOL(firstTimeInitializationRan, false);
#endif
...
#else
    WKC_DEFINE_STATIC_BOOL(fontCacheInvalidationCallbackRegistered, false);
#endif
...
#if ENABLE(IOS_TOUCH_EVENTS) || (PLATFORM(WKC) && ENABLE(TOUCH_EVENTS))
...
#if PLATFORM(WKC)
    m_userStyleSheet = TextResourceDecoder::create("text/css", PAL::UTF8Encoding())->decodeAndFlush(data->data(), data->size());
#else
