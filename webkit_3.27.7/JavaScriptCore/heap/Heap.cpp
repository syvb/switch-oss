...
#else
    WKC_DEFINE_STATIC_DOUBLE(maxPauseMS, 0);
#endif
...
void Heap::finalizeUnconditionalFinalizers()
{
    ...
    finalizeMarkedUnconditionalFinalizers<FunctionExecutable>(functionExecutableSpace.space);
...
void Heap::deleteAllUnlinkedCodeBlocks(DeleteAllCodeEffort effort)
{
    ...
    HeapIterationScope heapIterationScope(*this);
    unlinkedFunctionExecutableSpace.set.forEachLiveCell(
        [&] (HeapCell* cell, HeapCell::Kind) {
...
{
#if PLATFORM(WKC)
    WTF::PageBlock::changeReservedSize(static_cast<long>(capacity), OSAllocator::Usage::JSGCHeapPages);
#endif
#if ENABLE(RESOURCE_USAGE)
