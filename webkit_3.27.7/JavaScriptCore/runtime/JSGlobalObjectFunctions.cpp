...
#else
    static LazyNeverDestroyed<Bitmap<256>> doNotUnescapeWhenDecodingURI;

    if (doNotUnescapeWhenDecodingURI.isNull())
        doNotUnescapeWhenDecodingURI.construct(makeCharacterBitmap("#$&+,/:;=?@"));

    return JSValue::encode(decode(globalObject, callFrame->argument(0), doNotUnescapeWhenDecodingURI.get(), true));
#endif
}
...
#else
    static LazyNeverDestroyed<Bitmap<256>> emptyBitmap;
    
    if (emptyBitmap.isNull())
        emptyBitmap.construct();

    return JSValue::encode(decode(globalObject, callFrame->argument(0), emptyBitmap.get(), true));
#endif
}
...
JSC_DEFINE_HOST_FUNCTION(globalFuncEncodeURI, (JSGlobalObject* globalObject, CallFrame* callFrame))
{
...
#else
    static LazyNeverDestroyed<Bitmap<256>> doNotEscapeWhenEncodingURI;

    if (doNotEscapeWhenEncodingURI.isNull())
        doNotEscapeWhenEncodingURI.construct(makeCharacterBitmap(
            "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
            "abcdefghijklmnopqrstuvwxyz"
            "0123456789"
            "!#$&'()*+,-./:;=?@_~"
    ...
    return JSValue::encode(encode(globalObject, callFrame->argument(0), doNotEscapeWhenEncodingURI.get()));
#endif
}
...
JSC_DEFINE_HOST_FUNCTION(globalFuncEncodeURIComponent, (JSGlobalObject* globalObject, CallFrame* callFrame))
{
...
#else
    static LazyNeverDestroyed<Bitmap<256>> doNotEscapeWhenEncodingURIComponent;

    if (doNotEscapeWhenEncodingURIComponent.isNull())
        doNotEscapeWhenEncodingURIComponent.construct(makeCharacterBitmap(
            "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
            "abcdefghijklmnopqrstuvwxyz"
            "0123456789"
            "!'()*-._~"
    ...
    return JSValue::encode(encode(globalObject, callFrame->argument(0), doNotEscapeWhenEncodingURIComponent.get()));
#endif
}
...
#else
    static LazyNeverDestroyed<Bitmap<256>> doNotEscape;
    if (doNotEscape.isNull())
        doNotEscape.construct(makeCharacterBitmap(
            "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
            "abcdefghijklmnopqrstuvwxyz"
            "0123456789"
            "*+-./@_"
