...
StackBounds StackBounds::currentThreadStackBoundsInternal()
{
    void* ct = wkcThreadCurrentThreadPeer();
    void* origin = wkcThreadGetStackBasePeer(ct);
    unsigned int size = wkcThreadGetStackSizePeer(ct);

    void* cursp = (void *)&size;
    void* bound = nullptr;
    if (cursp < origin) {
        bound = (void *)(((char *)origin - size));
    }
    else {
        bound = (void *)(((char *)origin + size));
    }
