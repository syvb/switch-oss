...
#endif
#if PLATFORM(WKC)
#include <WebCore/inspector/InspectorInstrumentationWebKit.h>
#endif

// Match the parallel connection count used by the networking layer.
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_UINT_ZERO(maxRequestsInFlightPerHost);
#else
...
#if PLATFORM(WKC)
    if (InspectorInstrumentationWebKit::shouldInterceptRequest(resourceLoader->frame(), resourceLoader->request())) {
        InspectorInstrumentationWebKit::interceptRequest(*resourceLoader, [this, protectedResourceLoader = RefPtr { resourceLoader }](const ResourceRequest& request) {
            ResourceRequest temp;
            temp.setAsIsolatedCopy(request);
            protectedResourceLoader->setRequest(WTFMove(temp));
            scheduleLoad(protectedResourceLoader.get());
        });
        return;
    }
#endif
